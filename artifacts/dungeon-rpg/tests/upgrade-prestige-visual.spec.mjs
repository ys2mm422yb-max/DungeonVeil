...
async function seedInitialState(page, role, level) {
  await page.addInitScript(({ activeRole, activeLevel, roles }) => {
    ...
    localStorage.setItem('dungeon-veil-seen-unlocks-v1', JSON.stringify({ version: 2, initialized: true, equipment: ['ash-bow', 'ranger-quiver', 'ranger-cloak'], announcedEquipment: ['ash-bow', 'ranger-quiver', 'ranger-cloak'], relics: [], announcedRelics: [] }));
    localStorage.setItem('dungeon-veil-meta', JSON.stringify({ version: 4, rank: 20, xp: 420, dust: 999999, gold: 999999, owned: { 'ash-bow': { level: 1, copies: 1 }, 'ranger-quiver': { level: 1, copies: 1 }, 'ranger-cloak': { level: 1, copies: 1 } }, equipped: { bow: 'ash-bow', quiver: 'ranger-quiver', armor: 'ranger-cloak' }, cosmeticUnlocks: [], migrationCompensation: { gold: 0, dust: 0, copies: 0 }, rewardLedger: [], currentRunId: '' }));
    localStorage.setItem('dungeon-veil-companion-collection-v5', JSON.stringify({ version: 1, activeId: activeRole, companions: Object.fromEntries(roles.map(id => [id, { level: id === activeRole ? activeLevel : 5, unlockedAt: now - 60_000 }])), updatedAt: now }));
...
async function activateVariant(page, variant, reload) {
  ...
  await expect.poll(async () => Number(await marker.getAttribute('data-visible-count') ?? 0), { timeout: 60_000, message: `${variant.role} tier ${variant.level} must be visible in the actual shared renderer` }).toBeGreaterThan(0);

  const telemetry = async key => page.evaluate(name => document.documentElement.dataset[name], key);
  await expect.poll(() => telemetry('dungeonVeilCompanionUpgradeBinding'), { timeout: 60_000 }).toBe('in-run-companion-combat-mesh');
  ...
  await page.evaluate(({ role, level }) => {
    window.dispatchEvent(new CustomEvent('dungeon-veil-companion-action-v4', { detail: { ownerPlayerId: 'player', role, level, kind: 'attack', at: performance.now() } }));
  }, variant);
...
test('all companion roles and tiers keep prestige bound to the real combat model', async ({ page }, testInfo) => {
  ...
  for (const variant of CORE_VARIANTS) {
    ...
    await page.screenshot({ path: `test-results/visual-upgrade-prestige-tier-${variant.level}-${variant.role}-${testInfo.project.name}.png`, fullPage: false });
    reload = true;
  }
  for (const variant of MAX_ROLE_VARIANTS) await activateVariant(page, variant, true);

  await page.emulateMedia({ reducedMotion: 'reduce' });
  await expect.poll(() => page.evaluate(() => document.documentElement.dataset.dungeonVeilCompanionUpgradeStaticFallback)).toBe('true');
  await expect.poll(() => page.evaluate(() => document.documentElement.dataset.dungeonVeilCompanionUpgradeParticlesActive)).toBe('false');
  await page.screenshot({ path: `test-results/visual-upgrade-prestige-reduced-motion-${testInfo.project.name}.png`, fullPage: false });

  await page.emulateMedia({ reducedMotion: 'no-preference' });
  await expect.poll(() => page.evaluate(() => document.documentElement.dataset.dungeonVeilCompanionUpgradeStaticFallback)).toBe('false');
  await expect.poll(() => page.evaluate(() => document.documentElement.dataset.dungeonVeilCompanionUpgradeParticlesActive)).toBe('true');
  ...
  await expect.poll(() => page.evaluate(() => document.documentElement.dataset.dungeonVeilCompanionUpgradeStaticFallback)).toBe('true');
  await expect.poll(() => page.evaluate(() => document.documentElement.dataset.dungeonVeilCompanionUpgradeParticlesActive)).toBe('false');
  await page.screenshot({ path: `test-results/visual-upgrade-prestige-recovery-${testInfo.project.name}.png`, fullPage: false });
