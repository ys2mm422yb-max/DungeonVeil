...
test('restored collections, account entry and weekly elite contracts are visible', async ({ page }, testInfo) => {
  ...
  await page.getByRole('button', { name: /Visitenkarten|Calling Cards/i }).click();
  await expect(page.getByText(/Glutriss|Ember Rift/i).first()).toBeVisible();
  await page.getByRole('button', { name: /Avatare|Avatars/i }).click();
  await expect(page.getByText(/Aschenmaske|Ash Mask/i).first()).toBeVisible();
  await page.getByRole('button', { name: /Titel|Titles/i }).click();
  await expect(page.getByText(/Dungeonläufer|Dungeon Runner/i).first()).toBeVisible();

  await reloadMenu(page);
  await page.getByRole('button', { name: /Mehr|More/i }).click({ force: true });
  await page.getByRole('button', { name: /Online & Cloud/i }).click({ force: true });
  ...
  await reloadMenu(page);
  await page.getByRole('button', { name: /Aufträge|Quests/i }).first().click({ force: true });
  const content = page.getByTestId('quest-board-content');
  if (!await content.isVisible().catch(() => false)) await page.getByTestId('quest-board-toggle').click({ force: true });
