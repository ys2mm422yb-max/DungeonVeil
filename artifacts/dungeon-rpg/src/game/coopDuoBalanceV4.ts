...
import { DUO_BALANCE_V4 } from './buildBalanceV4';

export const DUO_NORMAL_HP_MULTIPLIER = DUO_BALANCE_V4.normalHp;
export const DUO_ELITE_HP_MULTIPLIER = DUO_BALANCE_V4.eliteHp;
export const DUO_BOSS_HP_MULTIPLIER = DUO_BALANCE_V4.bossHp;
export const DUO_ENEMY_ATTACK_MULTIPLIER = DUO_BALANCE_V4.enemyAttack;
export const DUO_ENEMY_COUNT_MULTIPLIER = DUO_BALANCE_V4.spawnFactor;
export const DUO_MOBILE_ENEMY_CAP = DUO_BALANCE_V4.mobileEnemyCap;
export const DUO_BOSS_SUPPORT_COUNT = 2;
export const DUO_CURRENCY_MULTIPLIER = 1.25;
export const DUO_DISCONNECT_HP_FACTOR = DUO_BALANCE_V4.disconnectHpFactor;
export const DUO_DISCONNECT_ATTACK_FACTOR = DUO_BALANCE_V4.disconnectAttackFactor;

const NORMAL_DEATH_MS = 680;

type EnemyStats = { hp: number; attack: number; defense: number; speed: number; size: number; color: string };
type DuoBalanceMemory = {
  roomKey: string;
  spawnedSupport: boolean;
  scaledEnemyIds: Set<string>;
  originalEnemyCount: number;
  disconnectAdjusted: boolean;
};
...
function hpMultiplier(enemy: Enemy): number {
  ...
  return enemy.isElite ? DUO_ELITE_HP_MULTIPLIER : DUO_NORMAL_HP_MULTIPLIER;
}
...
function createSupportEnemy(state: RunGameState, runSeed: number, index: number, spawnPointIndex: number): Enemy | null {
  const type = supportType(state.floor, index);
  const base = SUPPORT_STATS[type];
  const requestedPoints = isBossRoom(state.floor) ? 1 + DUO_BOSS_SUPPORT_COUNT : Math.min(DUO_MOBILE_ENEMY_CAP, Math.max(8, spawnPointIndex + 1));
  ...
  return {
    id: `duo-${runSeed}-${state.chapter}-${state.floor}-${index}`, type: 'enemy', enemyType: type,
    x: spawn.x, y: spawn.y, width: base.size, height: base.size, vx: 0, vy: 0,
    hp, maxHp: hp, attack: Math.max(1, Math.round(base.attack * attackScale)), defense: base.defense,
    speed: base.speed, color: base.color, state: 'chase', targetX: spawn.x, targetY: spawn.y,
    nextAttackTime: now + 700 + index * 120, flashUntil: 0, spawnTime: now + index * 80,
    lastAttackTime: 0, deathTime: 0, deathDuration: NORMAL_DEATH_MS, isDead: false,
...
function memoryFor(state: RunGameState, runSeed: number): DuoBalanceMemory {
  ...
  const created: DuoBalanceMemory = {
    roomKey, spawnedSupport: false, scaledEnemyIds: new Set<string>(),
    originalEnemyCount: state.enemies.length, disconnectAdjusted: false,
  };
...
export function ensureDuoRoomBalance(state: RunGameState, runSeed = 0): DuoRoomBalanceResult {
  ...
  return {
    roomKey: memory.roomKey, originalEnemyCount: memory.originalEnemyCount,
    finalEnemyCount: state.enemies.length,
    addedEnemyCount: Math.max(0, state.enemies.length - memory.originalEnemyCount),
    bossRoom, newlyScaledEnemyCount,
  };
}

export function applyDuoDisconnectFallback(state: RunGameState, runSeed = 0): boolean {
  const memory = memoryFor(state, runSeed);
  if (memory.disconnectAdjusted) return false;
  memory.disconnectAdjusted = true;
  for (const enemy of state.enemies) {
    const ratio = enemy.maxHp > 0 ? Math.max(0, Math.min(1, enemy.hp / enemy.maxHp)) : 1;
    enemy.maxHp = Math.max(1, Math.round(enemy.maxHp * DUO_DISCONNECT_HP_FACTOR));
    enemy.hp = Math.max(1, Math.round(enemy.maxHp * ratio));
    enemy.attack = Math.max(1, Math.round(enemy.attack * DUO_DISCONNECT_ATTACK_FACTOR));
  }
  return true;
}

export const applyDuoRoomBalance = ensureDuoRoomBalance;
