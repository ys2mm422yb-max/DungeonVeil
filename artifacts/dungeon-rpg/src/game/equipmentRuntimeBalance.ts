...
export function createEquipmentRuntimeBalanceState(): EquipmentRuntimeBalanceState {
  return {
    ...
    clawUntil: 0, crownStack: 0, crownInitialized: false, criticalDisposer: null,
  };
}
...
function reconcileIncomingDamage(engine: GameEngine, state: EquipmentRuntimeBalanceState) {
  ...
  const adjustedDamage = Math.max(1, Math.round(beforeDefense * (1 - defenseMitigation(player.defense, bossLike ? 0.44 : 0.52))));
...
function updateBoundedRelics(engine: GameEngine, state: EquipmentRuntimeBalanceState, time: number) {
  ...
  if (!state.crownInitialized) {
    ...
    for (let index = state.crownStack; index < stack; index++) player.attack = Math.max(1, Math.round(player.attack / 1.04 * 1.03));
  }
  state.crownStack = stack;
}

function normalizeAttackCooldown(engine: GameEngine, state: EquipmentRuntimeBalanceState, time: number) {
