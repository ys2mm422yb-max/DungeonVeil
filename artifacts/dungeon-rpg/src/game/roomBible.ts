...
import { VEIL_NEXUS_ROOMS, type VeilNexusSilhouette } from './veilNexusRooms';
import { resolvedRoomSetpieces } from './roomSetpieceResolver';
import { roomPropColliderFootprint } from './propPresentation3D';

export type RoomPhaseId = 'inhabited-mine' | 'abandoned-quarters' | 'ancient-ruins' | 'warden-veil' | 'meadow-forest' | 'darkwood-village' | 'fortress-ember' | 'golden-fracture' | 'shattered-observatory' | 'drowned-reliquary' | 'cinder-crown' | 'veil-nexus';
...
export type RoomBibleSpec = {
  room: number; nameDe: string; nameEn: string; phase: RoomPhaseId; silhouette: RoomSilhouette;
  heroObject: string; packs: RoomPack[]; keywords: string[]; forbiddenKeywords: string[];
  density: number; shell: RoomShell; portal: RoomBiblePoint; enemySpawns: RoomBiblePoint[];
  light: { background: number; fog: number; ambient: number; hemisphereSky: number; hemisphereGround: number; key: number; fill: number; exposure: number };
};
...
const PHASE_LIGHTS: Record<RoomPhaseId, RoomBibleSpec['light']> = {
  ...
  'cinder-crown': { background: 0x190806, fog: 0x240b08, ambient: 0xc69a8b, hemisphereSky: 0xe08b65, hemisphereGround: 0x210906, key: 0xff6b32, fill: 0x873a67, exposure: 1.14 },
  'veil-nexus': { background: 0x080516, fog: 0x120b28, ambient: 0xb7a9d8, hemisphereSky: 0x82d9ff, hemisphereGround: 0x100723, key: 0xd7b4ff, fill: 0x4ee8d5, exposure: 1.16 },
};
...
for (const blueprint of EXPANDED_ROOM_BLUEPRINTS) ROOM_BIBLE[blueprint.room] = room(blueprint.room, blueprint.nameDe, blueprint.nameEn, blueprint.phase, blueprint.silhouette, blueprint.heroObject, [...blueprint.packs], [...blueprint.keywords], [...blueprint.forbiddenKeywords], blueprint.density, blueprint.shell, { ...blueprint.portal }, blueprint.enemySpawns.map(point => ({ ...point })));

const GOLDEN_SILHOUETTES: Record<GoldenFractureSilhouette, RoomSilhouette> = { 'fractured-causeway': 'three-lane', 'split-sanctum': 'diagonal', 'crescent-forge': 's-curve', 'shattered-crossing': 'cross', 'twin-vaults': 'tri-island', 'spiral-descent': 'orbit', 'broken-crown': 'ring', 'veil-bridges': 's-lane', 'sunken-throne': 'axial', 'boss-oculus': 'arena' };
for (const spec of Object.values(GOLDEN_FRACTURE_ROOMS)) ROOM_BIBLE[spec.room] = room(spec.room, spec.titleDe, spec.titleEn, 'golden-fracture', GOLDEN_SILHOUETTES[spec.silhouette], spec.room === 60 ? 'goldenes Oculus mit vier Phasenankern' : `gebrochene Goldstruktur: ${spec.silhouette}`, ['resources', 'halloween'], ['gold', 'crystal', 'rift', 'floating', 'rune', 'pillar', spec.hazard], ['forest', 'village', 'bed', 'barrel_stack'], spec.room === 60 ? 1 : 3, 'veil', { ...spec.portal }, spec.enemySpawns.map(point => ({ ...point })));
const OBSERVATORY_SILHOUETTES: Record<ObservatorySilhouette, RoomSilhouette> = { 'fallen-orrery': 'orbit', 'meridian-split': 'tri-island', 'lens-graveyard': 'diagonal', 'astral-causeway': 'three-lane', 'clock-of-ash': 'cross', 'silent-ephemeris': 'ring', 'comet-archive': 'diagonal', 'parallax-vault': 'tri-island', 'last-calculation': 'orbit', 'astronomer-crown': 'arena' };
...
const RELIQUARY_SILHOUETTES: Record<ReliquarySilhouette, RoomSilhouette> = { 'sunken-threshold': 'three-lane', 'hall-of-chains': 'three-lane', 'broken-archives': 'tri-island', 'breathless-chapel': 'ring', 'green-vault': 'diagonal', 'reliquary-graves': 'cross', 'bell-beneath-water': 'orbit', 'drowned-procession': 's-curve', 'last-dry-ground': 'tri-island', 'leviathan-basin': 'arena' };
...
function resolvedEnemySpawns(roomNumber: number, spec: RoomBibleSpec): RoomBiblePoint[] {
  const blockers = resolvedRoomSetpieces(roomNumber).flatMap(piece => { const footprint = roomPropColliderFootprint(piece); return footprint ? [{ x: piece.x, z: piece.z, halfW: footprint.width / 2, halfH: footprint.height / 2 }] : []; });
  ...
  if (roomNumber % 10 === 0) { const bossSpawn = [...spec.enemySpawns, ...BOSS_SPAWN_CANDIDATES].find(point => valid(point, BOSS_SPAWN_CLEARANCE, 1.8)); if (!bossSpawn) throw new Error('Raum ' + roomNumber + ' besitzt keinen sicheren Boss-Spawnpunkt.'); return [{ ...bossSpawn }]; }
  ...
  for (const z of SPAWN_GRID_Z) { for (const x of SPAWN_GRID_X) { if (selected.length >= 8) break; add({ x, z }); } if (selected.length >= 8) break; }
...
export function roomBibleSpec(roomNumber: number): RoomBibleSpec {
  const safeRoom = Math.max(1, Math.min(100, Math.floor(roomNumber)));
  const cached = RESOLVED_ROOM_SPECS.get(safeRoom); if (cached) return cached;
  ...
  RESOLVED_ROOM_SPECS.set(safeRoom, resolved); return resolved;
}
