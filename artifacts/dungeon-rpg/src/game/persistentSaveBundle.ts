...
const CORE_BUNDLE_KEYS = [
  ...
  'dungeon-veil-optional-equipment-v1',
...
export function bundleActivityTimestamp(bundle: DungeonVeilSaveBundle): number {
  const save = parseBundleValue(bundle, 'dungeon-veil-save');
  const profile = parseBundleValue(bundle, 'dungeon-veil-player-profile-v1');
  const optionalEquipment = parseBundleValue(bundle, 'dungeon-veil-optional-equipment-v1');
  const legacyOptionalLoadout = parseBundleValue(bundle, 'dungeon-veil-optional-loadout-v1');
  ...
  return Math.max(
    ...
    number(optionalEquipment.updatedAt),
    number(legacyOptionalLoadout.updatedAt),
