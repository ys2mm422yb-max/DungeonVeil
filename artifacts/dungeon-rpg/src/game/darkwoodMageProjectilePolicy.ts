...
type RuntimeWindup = { hitAt: number; range: number; archetype: string; index: number };
type PendingProjectile = { id: string; enemyId: string; room: number; impactAt: number; targetX: number; targetY: number; damage: number; index: number };
...
function resolveProjectileImpact(engine: RuntimeEngine, projectile: PendingProjectile, time: number): void {
  ...
  engine.state.effects.push({ id: `darkwood-mage-impact-${projectile.id}`, x: projectile.targetX, y: projectile.targetY, radius: 0, maxRadius: 34, color: ARCANE_CORE, lifeTime: 0, maxLifeTime: 260, type: 'circle', element: 'arcane' });
  engine.state.particles.push(...makeHitSpark(projectile.targetX, projectile.targetY, ARCANE_COLOR, 14));
  if (missDistance > DARKWOOD_IMPACT_RADIUS || time <= player.invincibleUntil) return;
  player.hp -= projectile.damage;
  player.lastHitTime = time;
  if (skillRank(engine.state.runSkills, 'defense') > 0) player.lastGuardTime = time;
  engine.state.damageNumbers.push({ id: `hit-${time}-${projectile.index}`, x: player.x + 16 + (Math.random() - 0.5) * 14, y: player.y - 8, value: `-${projectile.damage}`, color: ARCANE_COLOR, lifeTime: 0, maxLifeTime: 800, scale: 1.18 });
}
...
function installDarkwoodMageProjectilePolicy(): void {
  ...
  prototype.updateEnemies = function updateEnemiesWithVisibleDarkwoodProjectiles(this: RuntimeEngine, dt: number, time: number): void {
    ...
    for (const [id, projectile] of projectiles) {
      if (projectile.room !== this.state.floor) { projectiles.delete(id); continue; }
      if (time >= projectile.impactAt) { projectiles.delete(id); resolveProjectileImpact(this, projectile, time); }
    }
    const previousAttackTimes = new Map<string, number>();
    for (const enemy of this.state.enemies as Enemy[]) if (isDarkwoodMage(this, enemy)) previousAttackTimes.set(enemy.id, enemy.lastAttackTime);
    ...
    for (const enemy of this.state.enemies as Enemy[]) {
      ...
      this.state.effects.push({ id: `darkwood-mage-warning-${time}-${enemy.id}`, x: enemyX, y: enemyY, radius: 0, maxRadius: 38, color: ARCANE_COLOR, lifeTime: 0, maxLifeTime: DARKWOOD_WINDUP_MS, type: 'circle', element: 'arcane' });
      this.state.effects.push({ id: `darkwood-mage-target-${time}-${enemy.id}`, x: playerX, y: playerY, radius: 0, maxRadius: DARKWOOD_IMPACT_RADIUS, color: ARCANE_CORE, lifeTime: 0, maxLifeTime: DARKWOOD_WINDUP_MS, type: 'circle', element: 'arcane' });
    }
  };
  ...
  prototype.resolveEnemyAttack = function resolveVisibleDarkwoodMageAttack(this: RuntimeEngine, enemy: Enemy, windup: RuntimeWindup, time: number): void {
    if (!isDarkwoodMage(this, enemy)) { originalResolveEnemyAttack.call(this, enemy, windup, time); return; }
    ...
    if (distance > DARKWOOD_RANGE * 1.05 || this.shotPathBlocked(fromX, fromY, targetX, targetY, 0.08)) return;
    ...
    this.state.effects.push({ id: `darkwood-mage-projectile-${projectileId}`, x: fromX, y: fromY, radius: 0, maxRadius: distance, color: ARCANE_COLOR, lifeTime: 0, maxLifeTime: travelMs, type: 'beam', angle, width: 12, element: 'arcane', fromEnemyId: enemy.id });
    this.state.effects.push({ id: `darkwood-mage-projectile-core-${projectileId}`, x: fromX, y: fromY, radius: 0, maxRadius: distance, color: ARCANE_CORE, lifeTime: 0, maxLifeTime: travelMs, type: 'beam', angle, width: 5, element: 'arcane', fromEnemyId: enemy.id });
    this.state.particles.push(...makeHitSpark(fromX, fromY, ARCANE_COLOR, 10));
    projectileStore(this).set(projectileId, { id: projectileId, enemyId: enemy.id, room: this.state.floor, impactAt: time + travelMs, targetX, targetY, damage, index: windup.index });
  };
}

installDarkwoodMageProjectilePolicy();

export const DARKWOOD_MAGE_PROJECTILE_POLICY = Object.freeze({ roomRange: [DARKWOOD_MIN_ROOM, DARKWOOD_MAX_ROOM] as const, attackRange: DARKWOOD_RANGE, windupMs: DARKWOOD_WINDUP_MS, attackCycleMs: DARKWOOD_ATTACK_CYCLE_MS, damageScale: DARKWOOD_DAMAGE_SCALE, projectileTravelMs: [DARKWOOD_PROJECTILE_MIN_MS, DARKWOOD_PROJECTILE_MAX_MS] as const, impactRadius: DARKWOOD_IMPACT_RADIUS });