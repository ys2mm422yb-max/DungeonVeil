...
function chooseWaypoint(context: NavigationContext, state: MageNavigationState, desired: Direction): Direction | null {
  ...
  const preferredSide = state.forcedAlternateUntil && context.time < state.forcedAlternateUntil ? -context.side : context.side;
...
function angularFallback(context: NavigationContext, state: MageNavigationState, desired: Direction): Direction {
  const preferred = state.forcedAlternateUntil && context.time < state.forcedAlternateUntil ? -context.side : context.side;
...
export function resolveMageObstacleDirection(context: NavigationContext, state: MageNavigationState): Direction {
  ...
  const desired = state.forcedAlternateUntil && context.time < state.forcedAlternateUntil
    ? context.alternate
    : context.desired;
  ...
  return angularFallback(context, state, desired);
}
