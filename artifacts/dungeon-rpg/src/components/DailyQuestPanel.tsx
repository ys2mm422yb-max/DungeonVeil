      ...
      <button type="button" onPointerDown={event => { event.preventDefault(); event.stopPropagation(); setOpen(value => !value); }} className="flex w-full items-center gap-3 px-4 py-3 text-left active:scale-[.985]">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center justify-between gap-2 text-[8px] font-black uppercase tracking-[.22em] text-amber-100/55">
            <span>{hasGold ? 'Goldener Tagesriss' : 'Tagesriss'}</span>
            <span>{completed}/3</span>
          </div>
          {activeTask && <>
            <div className="mt-1 truncate text-[11px] font-black text-white/82">{activeTask.title}</div>
            <div className="mt-1 h-1 overflow-hidden rounded-full bg-white/8"><div className="h-full rounded-full bg-amber-300/80" style={{ width: `${activeTask.target ? activeProgress / activeTask.target * 100 : 0}%` }} /></div>
          </>}
        </div>
        ...
        <div className="border-t border-white/8 px-4 pb-4 pt-2.5">
          ...
          <div className="space-y-2">
            {tasks.map(task => {
              ...
              return (
                <div key={task.id} className={`rounded-xl border px-3 py-2.5 ${claimed ? 'border-emerald-300/15 bg-emerald-400/[.06]' : task.gold ? 'border-amber-200/30 bg-amber-300/[.08]' : 'border-white/8 bg-black/25'}`}>
                  <div className="flex items-start justify-between gap-3">
                    <div><div className={`text-[10px] font-black ${task.gold ? 'text-amber-100' : 'text-white/82'}`}>{task.gold ? '✹ ' : ''}{task.title}</div><div className="mt-0.5 text-[7px] font-bold text-white/40">{task.description}</div></div>
