...
export function DailyQuestPanel({ compact = false, defaultOpen = false }: { compact?: boolean; defaultOpen?: boolean }) {
  ...
  return <div data-testid="daily-quest-panel" className={`overflow-hidden rounded-2xl border border-amber-300/20 bg-[linear-gradient(120deg,rgba(73,48,16,.72),rgba(9,8,8,.9))] shadow-[0_12px_34px_rgba(0,0,0,.32)] ${compact ? '' : 'backdrop-blur-xl'}`}>
    <button data-testid="quest-board-toggle" type="button" onPointerDown={event => { event.preventDefault(); event.stopPropagation(); setOpen(value => !value); }} className="flex min-h-12 w-full items-center gap-3 px-4 py-3 text-left active:scale-[.985]">
      ...
      <section data-testid="quest-elite-section" className="mt-3 overflow-hidden rounded-xl border border-fuchsia-300/22 bg-[linear-gradient(135deg,rgba(91,28,116,.25),rgba(13,9,20,.5))] p-3">
        ...
        <div className="mt-3 space-y-2">{weeklyTasks.map(quest => {
          ...
          return <article key={quest.id} data-testid="weekly-elite-card" className="rounded-xl border border-fuchsia-300/12 bg-black/24 p-3">
            ...
            <button data-testid="weekly-elite-claim" type="button" disabled={!complete || claimed} onPointerDown={event => { event.preventDefault(); event.stopPropagation(); if (complete && !claimed) claimWeekly(quest); }} className={`mt-2 min-h-11 w-full rounded-lg border px-3 py-2.5 text-[7px] font-black uppercase tracking-[.14em] touch-manipulation ${claimed ? 'border-emerald-300/16 bg-emerald-400/[.06] text-emerald-100' : complete ? 'border-fuchsia-300/24 bg-fuchsia-500/12 text-fuchsia-100 active:scale-[.98]' : 'border-white/8 bg-black/20 text-white/24'}`}>{claimed ? (de ? 'Belohnung erhalten' : 'Reward claimed') : complete ? (de ? 'Belohnung abholen' : 'Claim reward') : (de ? 'Noch offen' : 'In progress')}</button>
          </article>;
        })}</div>
      </section>

      <section data-testid="quest-completed-section" className="mt-3 overflow-hidden rounded-xl border border-white/8 bg-black/16">
        <button data-testid="quest-completed-toggle" type="button" onPointerDown={event => { event.preventDefault(); event.stopPropagation(); setCompletedOpen(value => !value); }} className="flex min-h-11 w-full touch-manipulation items-center justify-between px-3 py-2.5 text-[7px] font-black uppercase tracking-[.18em] text-white/38 active:scale-[.99]"><span>{de ? 'Erledigte Aufträge' : 'Completed Quests'} · {completedStandardTasks.length}</span><span className={`text-base transition-transform ${completedOpen ? 'rotate-90' : ''}`}>›</span></button>
