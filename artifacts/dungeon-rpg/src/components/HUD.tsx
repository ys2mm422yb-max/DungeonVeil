...
export function HUD({gameState:g,onPause}:Props){
 ...
 return <div className="fixed inset-0 z-40 pointer-events-none select-none" data-testid="run-hud">
  <div className={`absolute flex items-start justify-between ${tabletLandscape?'left-6 right-6 top-5':'left-3 right-3 top-3'}`} style={{paddingTop:'env(safe-area-inset-top)',paddingLeft:'env(safe-area-inset-left)',paddingRight:'env(safe-area-inset-right)'}}>
   <div className={`${tabletLandscape?'w-[236px] p-3.5':'w-[200px] p-3'} rounded-2xl border border-white/10 bg-black/55 shadow-xl backdrop-blur-sm`}>
    <div className={`mb-2 flex items-center justify-between font-black tracking-[.18em] text-white/75 ${tabletLandscape?'text-[11px]':'text-[10px]'}`}><span>KAPITEL {g.chapter}</span><span>RAUM {g.floor}/{CHAPTER_ROOMS}</span></div>
    <Bar v={p.hp} m={p.maxHp} c="#cb463d"/>
    <div className={`mt-2 flex items-center justify-between border-t border-white/8 pt-2 font-bold tracking-[.16em] text-white/45 ${tabletLandscape?'text-[9px]':'text-[8px]'}`}><span>WALDLÄUFER</span><span>{gifts} GABEN</span></div>
    {activeCombatSkills.length>0&&<div className="mt-2 grid grid-cols-3 gap-1">
     {activeCombatSkills.map(skill=><div key={skill.key} className={`min-w-0 truncate rounded-full border px-1.5 py-1 text-center font-black tracking-[.04em] ${tabletLandscape?'text-[6.5px]':'text-[5.5px]'} ${skill.tone}`}><span className={`mr-0.5 ${tabletLandscape?'text-[8px]':'text-[7px]'}`}>{skill.icon}</span>{skill.label} {skill.rank}</div>)}
    </div>}
    {modifier&&<div className={`mt-2 truncate rounded-full border border-violet-300/15 bg-violet-500/[.07] px-2 py-1 text-center font-black tracking-[.18em] text-violet-100/65 ${tabletLandscape?'text-[7px]':'text-[6px]'}`}>{modifier}</div>}
   </div>
   <button type="button" onPointerDown={e=>{e.preventDefault();e.stopPropagation();onPause()}} className={`pointer-events-auto grid place-items-center rounded-full border border-white/15 bg-black/55 font-black text-white/85 backdrop-blur-sm active:scale-90 ${tabletLandscape?'h-14 w-14 text-base':'h-12 w-12 text-sm'}`} data-ui-control>Ⅱ</button>
  </div>
  <div className={`absolute ${rightEdge} top-[max(5.25rem,calc(env(safe-area-inset-top)+4.25rem))] max-w-[42vw] truncate rounded-full border px-3 py-1.5 font-black tracking-[.14em] backdrop-blur-sm ${tabletLandscape?'text-[9px]':'text-[8px]'} ${g.roomClearReady?'border-violet-300/30 bg-violet-500/15 text-violet-100':boss?'border-red-300/30 bg-red-950/55 text-red-100':hunt?'border-amber-300/35 bg-amber-950/58 text-amber-100':'border-white/10 bg-black/46 text-white/65'}`}>{enemyText}</div>
  {hunt&&!boss&&<div className={`absolute ${rightEdge} top-[max(8.2rem,calc(env(safe-area-inset-top)+7rem))] w-[min(41vw,178px)] rounded-xl border border-amber-300/22 bg-black/70 px-3 py-2 shadow-[0_12px_34px_rgba(0,0,0,.42)] backdrop-blur-md`}>
  ...
  {boss&&<div className={`absolute ${rightEdge} top-[max(8.15rem,calc(env(safe-area-inset-top)+6.95rem))] w-[min(41vw,178px)] rounded-xl border border-red-300/20 bg-black/72 px-3 py-2.5 shadow-[0_14px_40px_rgba(0,0,0,.45)] backdrop-blur-md`}>
  ...
  {hintVisible&&<div className={`absolute ${rightEdge} top-[max(12rem,calc(env(safe-area-inset-top)+10.8rem))] max-w-[42vw] rounded-xl border border-orange-300/35 bg-black/75 px-3 py-2 text-center text-[8px] font-black tracking-[.12em] text-orange-100 shadow-xl`}>{exitHint}</div>}
 </div>
}
