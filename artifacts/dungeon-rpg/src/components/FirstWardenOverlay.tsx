  ...
  return (
    <div className={`pointer-events-none fixed inset-0 z-[80] flex items-center justify-center transition-opacity duration-500 ${visible ? 'opacity-100' : 'opacity-0'}`}>
      ...
      <div className="relative mx-6 w-full max-w-sm text-center">
        <div className={`text-[8px] font-black uppercase tracking-[.45em] ${intro ? 'text-violet-200/60' : 'text-amber-200/60'}`}>{intro ? 'REGIONSWÄCHTER · RAUM 20' : 'ERSTE REGION GEBROCHEN'}</div>
        <div className={`mt-3 font-serif text-[2.1rem] font-black leading-none tracking-[.08em] drop-shadow-[0_8px_28px_rgba(0,0,0,.85)] ${intro ? 'text-violet-100' : 'text-[#e7c37a]'}`}>{intro ? 'DER ERSTE WÄCHTER' : 'DER WÄCHTER IST GEFALLEN'}</div>
        <div className="mx-auto mt-4 h-px w-28 bg-gradient-to-r from-transparent via-white/45 to-transparent" />
        <div className="mt-3 text-[9px] font-black uppercase tracking-[.22em] text-white/45">{intro ? 'Der Schleierkern antwortet auf deinen Schritt.' : 'Berge die Beute und verlasse die Wächterhalle.'}</div>
      </div>
    </div>
  );
}
