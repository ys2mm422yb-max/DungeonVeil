...
import { unequipActiveCompanionV5 } from '../game/companionOptionalSelection';
...
function CompanionPortrait({ role, locked = false }: { role: CompanionRoleV4; locked?: boolean }) {
  ...
  return <div className={`relative grid h-20 w-20 shrink-0 place-items-center overflow-hidden rounded-[22px] border ${ROLE_TONE[role]} ${locked ? 'grayscale opacity-45' : ''}`}>
    ...
    {definition.species === 'veil-lynx' && <svg viewBox="0 0 120 120" aria-hidden="true" className={common}><path d="M22 21 43 38l17-6 17 6 21-17-7 34c6 8 9 17 9 28 0 22-18 35-40 35S20 105 20 83c0-11 3-20 9-28l-7-34Z" fill="#151725" stroke="currentColor" strokeWidth="3" strokeLinejoin="round"/><path d="M34 75c10-8 18-11 26-11s16 3 26 11l-7 20-19 13-19-13-7-20Z" fill="#243447"/><path d="m39 77 13 5M81 77l-13 5M51 94l9 5 9-5" fill="none" stroke="currentColor" strokeWidth="3.2" strokeLinecap="round"/><circle cx="47" cy="72" r="3.4" fill="#e9fbff"/><circle cx="73" cy="72" r="3.4" fill="#e9fbff"/></svg>}
    {definition.species === 'ember-raven' && <svg viewBox="0 0 120 120" aria-hidden="true" className={common}><path d="M60 19c18 0 29 13 29 31 0 9-3 16-8 22l18 17-27-7-12 26-12-26-27 7 18-17c-5-6-8-13-8-22 0-18 11-31 29-31Z" fill="#1b1720" stroke="currentColor" strokeWidth="3"/><path d="m59 42 25 13-24 8-19-8 18-13Z" fill="#43261f"/><path d="M35 55 13 68l28 4M85 55l22 13-28 4" fill="none" stroke="currentColor" strokeWidth="6" strokeLinecap="round"/><circle cx="51" cy="48" r="3" fill="#fff0d6"/><circle cx="69" cy="48" r="3" fill="#fff0d6"/></svg>}
    {definition.species === 'rune-sentinel' && <svg viewBox="0 0 120 120" aria-hidden="true" className={common}><path d="M38 19h44l10 25-8 18 9 39-33 14-33-14 9-39-8-18 10-25Z" fill="#1a2626" stroke="currentColor" strokeWidth="3" strokeLinejoin="round"/><path d="M43 43h34l7 18-24 22-24-22 7-18Z" fill="#294139"/><path d="m60 34 8 14-8 12-8-12 8-14ZM43 90h34" fill="none" stroke="currentColor" strokeWidth="4" strokeLinejoin="round"/><circle cx="49" cy="58" r="3" fill="#e9fff2"/><circle cx="71" cy="58" r="3" fill="#e9fff2"/></svg>}
    {definition.species === 'lantern-wisp' && <svg viewBox="0 0 120 120" aria-hidden="true" className={common}><circle cx="60" cy="59" r="31" fill="#252218" stroke="currentColor" strokeWidth="3"/><circle cx="60" cy="59" r="18" fill="currentColor" opacity=".32"/><path d="M60 14 68 31 60 39 52 31 60 14ZM60 104l-8-17 8-8 8 8-8 17ZM15 59l17-8 8 8-8 8-17-8ZM105 59l-17 8-8-8 8-8 17 8Z" fill="currentColor" opacity=".7"/><path d="M48 61c8-7 16-7 24 0M52 73c5 4 11 4 16 0" fill="none" stroke="#fff8d8" strokeWidth="3" strokeLinecap="round"/></svg>}
    {definition.species === 'dusk-drake' && <svg viewBox="0 0 120 120" aria-hidden="true" className={common}><path d="M60 20 78 33l21-8-10 24 12 20-24-3-17 36-17-36-24 3 12-20-10-24 21 8 18-13Z" fill="#20182d" stroke="currentColor" strokeWidth="3" strokeLinejoin="round"/><path d="M42 54c11-8 25-8 36 0l-5 25-13 14-13-14-5-25Z" fill="#382450"/><path d="m45 56 12 7M75 56l-12 7M53 80l7 5 7-5" fill="none" stroke="currentColor" strokeWidth="3.2" strokeLinecap="round"/><circle cx="50" cy="55" r="3.2" fill="#f4eaff"/><circle cx="70" cy="55" r="3.2" fill="#f4eaff"/></svg>}
...
export function CompanionManagementPanel({ language = 'de', embedded = false }: Props) {
  ...
  return <section data-testid="companion-management-panel" data-embedded={embedded ? 'true' : 'false'} data-companion-species={active?.species ?? 'none'} data-selection-surface="pre-run-only" className={shellClass}>
    <header className="flex items-end justify-between gap-3">
      <div><div className="text-[7px] font-black uppercase tracking-[.3em] text-violet-100/42">{de ? 'BEGLEITER-SAMMLUNG' : 'COMPANION COLLECTION'}</div><h2 className="mt-1 text-2xl font-black text-violet-50">{de ? 'Gefährten des Schleiers' : 'Allies of the Veil'}</h2><p className="mt-1.5 max-w-2xl text-[9px] leading-relaxed text-white/46">{de ? 'Jeder Begleiter muss gefunden werden. Auswahl, Ablegen und Verbesserung gelten für den nächsten Run.' : 'Every companion must be found. Selection, unequipping and upgrades apply to the next run.'}</p></div>
    ...
    <article data-testid="companion-active-role" data-companion-role={collection.activeId ?? 'none'} className="mt-3 rounded-[24px] border border-violet-200/22 bg-[radial-gradient(circle_at_12%_10%,rgba(139,92,246,.2),rgba(255,255,255,.025)_58%)] p-3">
      {active && activeProgress ? <div>
        <div className="grid grid-cols-[80px_minmax(0,1fr)] items-center gap-3"><CompanionPortrait role={active.id} /><div className="min-w-0"><div className="text-[7px] font-black uppercase tracking-[.2em] text-violet-100/42">{de ? 'FÜR DEN NÄCHSTEN RUN AKTIV' : 'ACTIVE FOR THE NEXT RUN'}</div><div className="mt-1 text-lg font-black text-white">{active.nameDe} <span className="text-white/46">· {de ? active.titleDe : active.titleEn}</span></div><div className="mt-1 text-[8px] leading-relaxed text-white/48">{de ? active.bonusDe : active.bonusEn}</div><div className={`mt-2 inline-flex rounded-full border px-2.5 py-1 text-[7px] font-black ${ROLE_TONE[active.id]}`}>{de ? 'STUFE' : 'LEVEL'} {activeProgress.level} / {COMPANION_MAX_LEVEL_V5}</div></div></div>
        <button data-testid="companion-unequip-button" type="button" onPointerDown={event => { event.preventDefault(); unequipActiveCompanionV5(); setNotice(de ? 'Der Begleiterplatz ist für den nächsten Run leer.' : 'The companion slot is empty for the next run.'); refresh(); }} className="mt-3 w-full rounded-xl border border-white/12 bg-black/28 px-3 py-2.5 text-[8px] font-black uppercase tracking-[.12em] text-white/62 active:scale-[.99]">{de ? 'BEGLEITER ABLEGEN' : 'UNEQUIP COMPANION'}</button>
      </div> : <div className="py-4 text-center"><div className="text-lg font-black text-white/82">{unlockedCount ? (de ? 'Kein Begleiter ausgewählt' : 'No companion selected') : (de ? 'Noch kein Begleiter gefunden' : 'No companion found yet')}</div><div className="mt-1 text-[8px] text-white/42">{unlockedCount ? (de ? 'Du kannst unten jederzeit wieder einen Gefährten auswählen.' : 'You can select an ally again below at any time.') : (de ? 'Der erste seltene Fund wird ab Kapitel 2 verfügbar.' : 'The first rare find becomes available from chapter 2.')}</div></div>}
    </article>
    ...
    <div className="mt-3 flex items-center justify-between"><div className="text-[7px] font-black uppercase tracking-[.2em] text-white/38">{de ? 'FUNDE UND VERBESSERUNGEN' : 'FINDS AND UPGRADES'}</div><div data-testid="companion-reserve-count" className="text-[7px] font-black text-white/38">{unlockedCount} / {COMPANION_ROLE_ORDER_V4.length} {de ? 'GEFUNDEN' : 'FOUND'}</div></div>
    ...
    <div data-testid="companion-reserve-grid" className="mt-2 grid grid-cols-1 gap-2 sm:grid-cols-2">
      {COMPANION_ROLE_ORDER_V4.map(role => {
        ...
        return <article key={role} data-testid={`companion-role-${role}`} data-unlocked={progress ? 'true' : 'false'} data-selected={selected ? 'true' : 'false'} className={`rounded-[22px] border p-3 ${progress ? ROLE_TONE[role] : 'border-white/[.08] bg-black/24 text-white/48'}`}>
          <div className="flex items-start gap-3"><CompanionPortrait role={role} locked={!progress} /><div className="min-w-0 flex-1"><div className="flex items-start justify-between gap-2"><div><div className="text-sm font-black text-white/92">{definition.nameDe}</div><div className="text-[7px] font-black uppercase tracking-[.14em] text-current/70">{de ? definition.titleDe : definition.titleEn}</div></div>{progress && <span className="rounded-full border border-white/10 bg-black/28 px-2 py-1 text-[7px] font-black">{de ? 'ST.' : 'LV.'} {progress.level}</span>}</div><p className="mt-2 text-[7px] leading-relaxed text-white/45">{de ? definition.bonusDe : definition.bonusEn}</p>{!progress && <div className="mt-2 text-[7px] font-black uppercase tracking-[.12em] text-white/38">{eligible ? (de ? 'SELTENER FUND VERFÜGBAR' : 'RARE FIND AVAILABLE') : `${de ? 'AB KAPITEL' : 'FROM CHAPTER'} ${definition.unlockChapter}`}</div>}</div></div>
