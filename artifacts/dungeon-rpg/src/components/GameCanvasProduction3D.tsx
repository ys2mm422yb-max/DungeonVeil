...
export function GameCanvasProduction3D({ gameState }: { gameState: GameState }) {
  ...
  useEffect(() => {
    ...
    const material = (color: number, roughness = 0.8, emissive = 0, emissiveIntensity = 0) =>
    ...
    const makeSlime = (enemy: GameState['enemies'][number]) => {
      ...
      const body = add(group, new THREE.Mesh(new THREE.SphereGeometry(0.44, 14, 9), material(color, 0.46, color, 0.12)));
      body.scale.set(1.12, squash, 1);
      body.position.y = 0.26;
      body.userData.baseY = body.position.y;
      body.userData.baseScaleY = squash;
      const eyeMaterial = material(0x140d0a, 0.3);
      for (const x of [-0.13, 0.13]) {
        const eye = add(group, new THREE.Mesh(new THREE.SphereGeometry(0.052, 6, 5), eyeMaterial));
    ...
    const syncArrows = (state: GameState) => {
      ...
      for (const shot of shots) {
        ...
        arrow.position.set(wx(shot.x) + Math.cos(angle) * travel * progress, 0.82, wz(shot.y) + Math.sin(angle) * travel * progress);
    ...
    const makeItem = (type: string) => {
      const group = new THREE.Group();
      if (type === 'xp_orb') {
        const gem = add(group, new THREE.Mesh(new THREE.OctahedronGeometry(0.16, 0), material(0x69d9ff, 0.2, 0x1d8ccb, 1.3)));
        gem.rotation.z = 0.35;
        ...
        add(group, new THREE.Mesh(new THREE.SphereGeometry(0.16, 8, 7), material(0xa52c40, 0.35, 0x5d1322, 0.4)));
        const neck = add(group, new THREE.Mesh(new THREE.CylinderGeometry(0.06, 0.08, 0.16, 7), material(0xa52c40, 0.4)));
        neck.position.y = 0.16;
        const cork = add(group, new THREE.Mesh(new THREE.CylinderGeometry(0.065, 0.065, 0.06, 7), material(0x6b4326, 1)));
    ...
    const syncWalls = (state: GameState) => {
      ...
      const rockMaterial = material(0x65594a, 0.98);
      const mossMaterial = material(0x526a37, 1);
      for (let y = 0; y < state.map.height; y++) {
        for (let x = 0; x < state.map.width; x++) {
          ...
          const rock = new THREE.Mesh(new THREE.DodecahedronGeometry(edge ? 0.42 : 0.34, 0), rockMaterial);
          ...
          wallGroup.add(rock);
          if (seed % 5 === 0) {
            const moss = new THREE.Mesh(new THREE.SphereGeometry(0.11, 6, 4), mossMaterial);
    ...
    const syncPortal = (state: GameState, now: number) => {
      const isClear = state.enemies.every(enemy => enemy.hp <= 0 || enemy.isDead) && state.status === 'playing';
      if (!isClear) {
        if (portal) {
          scene.remove(portal);
      ...
      if (!portal) {
        const group = new THREE.Group();
        const stone = material(0x40365a, 0.9);
    ...
    const syncEffects = (state: GameState) => {
      ...
      const activeIds = new Set(visible.map(effect => effect.id));
    ...
    const renderLoop = () => {
      ...
      if (hero) {
        hero.position.set(playerX, 0, playerZ);
        ...
        const nextAction = player.state === 'moving' ? runAction : idleAction;
    ...
    const boot = async () => {
      ...
      const makeTexture = (base: string, dark: string, light: string, dots: number) => {
        ...
        for (let i = 0; i < dots; i++) {
          ...
          ctx.beginPath();
          ctx.arc(random() * 256, random() * 256, radius, 0, Math.PI * 2);
          ctx.fill();
        }
      ...
      let decorSeed = 41827;
      const randomDecor = () => { decorSeed = (decorSeed * 1664525 + 1013904223) >>> 0; return decorSeed / 4294967296; };
      for (let i = 0; i < 58; i++) {
        const side = i % 4;
        let x = 0;
        let z = 0;
        if (side === 0) { x = -10.4 - randomDecor() * 5; z = -13 + randomDecor() * 28; }
        else if (side === 1) { x = 10.4 + randomDecor() * 5; z = -13 + randomDecor() * 28; }
        else if (side === 2) { x = -9 + randomDecor() * 18; z = -14.5 - randomDecor() * 5; }
        else { x = -9 + randomDecor() * 18; z = 14.5 + randomDecor() * 5; }
        const useRock = randomDecor() > 0.62;
        const object = (useRock ? rockProto : bushProto).clone(true);
        object.position.set(x, 0, z);
        object.rotation.y = randomDecor() * Math.PI * 2;
        object.scale.setScalar(useRock ? 0.55 + randomDecor() * 0.65 : 0.42 + randomDecor() * 0.58);
        object.traverse((node: any) => {
          if (node.isMesh) {
            node.castShadow = true;
            node.receiveShadow = true;
    ...
    boot().catch(error => console.error('Production 3D renderer failed', error));
