...
export function GameCanvasForestArena3D({ gameState }: { gameState: GameState }) {
  ...
  useEffect(() => {
    ...
    const rebuildArena = (state: GameState) => {
      ...
      for (let y = 0; y < state.map.height; y++) {
        for (let x = 0; x < state.map.width; x++) {
          ...
          // Small visual-only floor detail: never creates collision.
          if (seed > 0.9) {
    ...
    const slimeVariant = (enemy: GameState['enemies'][number]) => {
      let hash = 0;
      for (let index = 0; index < enemy.id.length; index++) hash = (hash * 31 + enemy.id.charCodeAt(index)) >>> 0;
      return { color: SLIME_COLORS[hash % SLIME_COLORS.length], size: 0.8 + ((hash >>> 3) % 5) * 0.12, squash: 0.52 + ((hash >>> 7) % 4) * 0.06 };
    };
    ...
    boot().catch(error => console.error('Forest arena renderer failed', error));
