...
export function GuildSocialPanel({ language, onClose, onOpenOnline }: Props) {
  ...
  if (!signedIn) return <section data-testid="guild-signed-out-panel" className="relative overflow-hidden rounded-3xl border border-amber-300/18 bg-[#0d0b08]/98 p-4 text-white shadow-2xl">
    ...
    <button data-testid="guild-close-button" type="button" aria-label={de ? 'Gilde schließen' : 'Close guild'} onClick={onClose} className="absolute right-3 top-3 z-10 grid h-9 w-9 place-items-center rounded-xl border border-white/12 bg-black/45 text-lg font-black text-white/72 active:scale-90">×</button>
    <header className="relative pr-12">
      ...
      <h2 className="mt-1 text-xl font-black text-amber-100">{de ? 'Gemeinsam durch den Schleier' : 'Together through the Veil'}</h2>
    ...
    <button type="button" onClick={onOpenOnline} className="relative mt-4 min-h-11 w-full rounded-xl border border-amber-300/30 bg-amber-500/12 px-3 text-[8px] font-black uppercase tracking-[.14em] text-amber-100 active:scale-[.99]">{de ? 'ONLINE & CLOUD ÖFFNEN' : 'OPEN ONLINE & CLOUD'}</button>
  </section>;
