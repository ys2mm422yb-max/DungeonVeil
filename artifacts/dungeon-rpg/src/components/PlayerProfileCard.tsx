  ...
  return <div className="fixed inset-0 z-[180] flex items-center justify-center overflow-hidden bg-black/82 px-4 pb-[max(12px,env(safe-area-inset-bottom))] pt-[max(12px,env(safe-area-inset-top))] backdrop-blur-md" role="dialog" aria-modal="true" aria-labelledby={titleId} data-testid="public-player-profile-dialog" onPointerDown={onClose}>
    <div data-testid="player-profile-card" className="relative max-h-full w-full max-w-sm overflow-y-auto overscroll-contain rounded-3xl border bg-[#08090c]/98 p-4 text-white shadow-2xl [-webkit-overflow-scrolling:touch]" style={{ borderColor: `${card.border}70`, boxShadow: `0 20px 70px rgba(0,0,0,.7),0 0 30px ${card.glow}` }} onPointerDown={event => event.stopPropagation()}>
      ...
      {!loading && !error && profile && <>
        ...
        <section data-testid="public-player-profile-cosmetics" className="mt-3 rounded-2xl border border-white/8 bg-white/[.02] p-3"><div className="text-[7px] font-black uppercase tracking-[.18em] text-white/35">{de ? 'AUSGEWÄHLTE SAMMLUNG' : 'EQUIPPED COLLECTION'}</div><div className="mt-2 grid grid-cols-3 gap-2 text-center">{[[de ? 'Avatar' : 'Avatar', de ? avatar.nameDe : avatar.nameEn, avatar.rarity], [de ? 'Titel' : 'Title', de ? title.nameDe : title.nameEn, title.rarity], [de ? 'Visitenkarte' : 'Calling card', de ? card.nameDe : card.nameEn, card.rarity]].map(([kind, name, rarity]) => <div key={String(kind)} className="min-w-0 rounded-xl border border-white/8 bg-black/25 p-2"><div className="text-[6px] font-black uppercase text-white/28">{kind}</div><div className="mt-1 truncate text-[7px] font-black text-white/70">{name}</div><div className="mt-1 text-[6px] uppercase text-amber-100/52">{rarityLabel(rarity as CosmeticRarity | undefined, de)}</div></div>)}</div></section>
