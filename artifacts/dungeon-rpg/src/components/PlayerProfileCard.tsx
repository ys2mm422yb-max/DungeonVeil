    ...
    <div data-testid="player-profile-card" className="relative max-h-full w-full max-w-sm overflow-y-auto overscroll-contain rounded-3xl border bg-[#08090c]/98 p-4 text-white shadow-2xl [-webkit-overflow-scrolling:touch]" style={{ borderColor: `${card.border}70`, boxShadow: `0 20px 70px rgba(0,0,0,.7),0 0 30px ${card.glow}` }} onPointerDown={event => event.stopPropagation()}>
      ...
      {!loading && !error && profile && <>
        <section data-testid="public-player-profile-header" className="mt-4 overflow-hidden rounded-2xl border p-4" style={{ background: card.background, borderColor: card.border, boxShadow: `0 0 22px ${card.glow}` }}>
          <div className="flex min-w-0 items-center gap-3"><ProfileAvatarPortrait avatar={avatar} className="h-20 w-20 shrink-0 rounded-2xl border border-white/20" /><div className="min-w-0 flex-1"><div className="truncate text-lg font-black text-white">{profile.display_name}</div><div className="mt-1 truncate text-[8px] font-black uppercase tracking-[.14em] text-white/66">{de ? title.nameDe : title.nameEn}</div><div className="mt-2 flex flex-wrap gap-1.5"><span className="rounded-full border border-amber-200/20 bg-black/24 px-2 py-1 text-[7px] font-black uppercase tracking-[.12em] text-amber-100">{de ? `Rang ${profile.current_rank}` : `Rank ${profile.current_rank}`}</span>{profile.guild_name && <span className="rounded-full border border-white/12 bg-black/24 px-2 py-1 text-[7px] font-black uppercase tracking-[.12em] text-white/62">[{profile.guild_tag ?? ''}] {profile.guild_name}</span>}</div><div className={`mt-2 text-[7px] font-black uppercase tracking-[.13em] ${profile.activity_state === 'run' ? 'text-emerald-200' : 'text-white/45'}`}>{activityLabel(profile, de)}</div></div></div>
