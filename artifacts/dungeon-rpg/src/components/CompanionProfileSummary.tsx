...
export function CompanionProfileSummary({ role, level = 1, language = 'de', testId = 'companion-profile-summary' }: {
  ...
  return <section data-testid={testId} data-companion-role={role} data-companion-species={definition.species} data-companion-level={level} className="grid gap-3 rounded-2xl border border-violet-300/14 bg-[radial-gradient(circle_at_15%_0%,rgba(139,92,246,.14),rgba(255,255,255,.02)_62%)] p-4 sm:grid-cols-[auto_minmax(0,1fr)_auto] sm:items-center">
    ...
    <div className="rounded-xl border border-white/8 bg-black/22 px-3 py-2 text-center"><div className="text-[5px] font-black uppercase tracking-[.12em] text-white/28">{de ? 'Stufe' : 'Level'}</div><div className="mt-1 text-sm font-black text-violet-100">{level}</div></div>
  </section>;
}
