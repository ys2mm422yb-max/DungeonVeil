...
type VisibleViewport = { width: number; height: number; left: number; top: number };
...
export function CombatStage({ gameState }: { gameState: GameState }) {
  ...
  const [viewport, setViewport] = useState<VisibleViewport>(() => visibleViewport());
  ...
  useEffect(() => {
    ...
    if (!isPlayerHit) {
      ...
      if (isHeavy) { try { navigator.vibrate?.(28); } catch {} }
    ...
    const compactSynergyBanner = () => {
      ...
      labels.forEach(label => {
        ...
        panel.querySelectorAll<HTMLElement>('div,p,span').forEach(child => {
          ...
          if (child !== label && child.textContent && child.textContent.length > 24) child.style.setProperty('font-size', '12px', 'important');
        });
  ...
  return (
    <div
      className="fixed overflow-hidden bg-[#100c0a]"
