...
export function GuildPanelMobile({ language, onClose, onOpenOnline, onOpenMemberProfile }: Props) {
  ...
  const stats = useMemo(() => ({ members: members.length, officers: members.filter(member => member.role === 'officer').length }), [members]);
  ...
  const leaveCurrentGuild = () => run(async () => {
    ...
    if (!window.confirm(disband ? (de ? 'Gilde endgültig auflösen? Dieser Schritt kann nicht rückgängig gemacht werden.' : 'Disband the guild permanently? This cannot be undone.') : (de ? 'Gilde wirklich verlassen?' : 'Really leave guild?'))) return;
  ...
  const createNewGuild = () => run(async () => {
    ...
    if (name.length < 3 || name.length > 32) throw new Error(de ? 'Der Gildenname muss 3 bis 32 Zeichen lang sein.' : 'The guild name must be 3 to 32 characters long.');
  ...
  return <div data-testid="guild-panel-shell" className="flex h-[min(64dvh,620px)] min-h-[360px] flex-col overflow-hidden rounded-3xl border border-amber-300/18 bg-[#0d0b08]/96 p-3 text-white shadow-2xl">
    ...
    {session && membership && <div className="flex min-h-0 flex-1 flex-col">
      ...
      {tab === 'overview' && <div data-testid="guild-overview-tab" className={`${scrollClass} space-y-3`}>
        <section className="grid grid-cols-3 gap-2">{[[stats.members, de ? 'Mitglieder' : 'Members'], [stats.officers, de ? 'Offiziere' : 'Officers'], [roleLabel(membership.role, de), de ? 'Deine Rolle' : 'Your role']].map(([value, label]) => <div key={String(label)} className="rounded-xl border border-amber-300/12 bg-amber-400/[.04] px-2 py-2 text-center"><div className="truncate text-[12px] font-black text-amber-100">{value}</div><div className="mt-1 truncate text-[6px] font-black uppercase tracking-[.12em] text-white/30">{label}</div></div>)}</section>
      ...
      {tab === 'members' && <section data-testid="guild-members-tab" className={`${scrollClass} space-y-2`}>
        <div className="flex items-center justify-between gap-3 px-1"><div className="text-[8px] font-black uppercase tracking-[.18em] text-amber-100/62">{de ? `${members.length} Mitglieder` : `${members.length} members`}</div><div className="text-[7px] text-white/34">{de ? 'Profil antippen' : 'Tap profile'}</div></div>
        {members.map(member => {
          ...
          return <article key={member.user_id} data-testid="guild-member-card" className="rounded-2xl border border-white/8 bg-white/[.025] p-2.5">
            <button type="button" data-testid="guild-member-profile-button" onClick={() => onOpenMemberProfile?.(member.user_id)} disabled={!onOpenMemberProfile} className="flex w-full min-w-0 items-center gap-3 rounded-xl p-1.5 text-left disabled:cursor-default active:scale-[.99]">
              <div className="grid h-11 w-11 shrink-0 place-items-center rounded-xl border border-cyan-300/16 bg-cyan-400/8 text-[10px] font-black text-cyan-100">{initials(name)}</div>
              <div className="min-w-0 flex-1"><div className="truncate text-[11px] font-black text-white/88">{name}</div><div className="mt-1 truncate text-[7px] uppercase tracking-[.12em] text-amber-100/52">{roleLabel(member.role, de)}{member.joined_at ? ` · ${formatJoined(member.joined_at, language)}` : ''}</div></div>
