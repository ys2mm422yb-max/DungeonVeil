...
function ActionButton({ label, onClick, disabled = false, primary = false, danger = false, compact = false }: {
  ...
  return <button
    ...
    disabled={disabled}
    onClick={onClick}
    className={`${compact ? 'min-h-8 px-2 py-1.5 text-[7px]' : 'min-h-10 px-3 py-2 text-[8px]'} rounded-xl border font-black uppercase tracking-[.13em] active:scale-[.98] ${style} ${disabled ? 'pointer-events-none opacity-35' : ''}`}
  >{label}</button>;
}
...
export function GuildPanelMobile({ language }: Props) {
  ...
  const leaveCurrentGuild = () => run(async () => {
    ...
    if (!window.confirm(disband
      ...
      : (de ? 'Gilde wirklich verlassen?' : 'Really leave the guild?'))) return;
  ...
  const tabButton = (key: GuildTab, label: string) => <button
    type="button"
    onClick={() => setTab(key)}
    className={`min-h-10 rounded-xl border px-2 text-[7px] font-black uppercase tracking-[.12em] active:scale-[.98] ${tab === key ? 'border-amber-300/35 bg-amber-500/15 text-amber-100' : 'border-white/8 bg-black/25 text-white/38'}`}
  >{label}</button>;
  ...
  return <div data-testid="guild-panel-shell" className="flex h-[min(64dvh,620px)] min-h-[360px] flex-col overflow-hidden rounded-3xl border border-amber-300/18 bg-[#0d0b08]/96 p-3 text-white shadow-2xl">
    ...
    {session && membership && <div className="flex min-h-0 flex-1 flex-col">
      ...
      {tab === 'overview' && <div data-testid="guild-overview-tab" className={`${scrollClass} space-y-3`}>
        <section className="grid grid-cols-3 gap-2">
          {[[stats.members, de ? 'Mitglieder' : 'Members'], [stats.officers, de ? 'Offiziere' : 'Officers'], [roleLabel(membership.role, de), de ? 'Deine Rolle' : 'Your role']].map(([value, label]) => <div key={String(label)} className="rounded-xl border border-amber-300/12 bg-amber-400/[.04] px-2 py-2 text-center">
            <div className="truncate text-[12px] font-black text-amber-100">{value}</div>
            <div className="mt-1 truncate text-[6px] font-black uppercase tracking-[.12em] text-white/30">{label}</div>
          </div>)}
        </section>

        <section className="space-y-2 rounded-2xl border border-white/8 bg-white/[.025] p-3">
          <div className="text-[7px] font-black uppercase tracking-[.18em] text-white/35">{de ? 'GILDENBESCHREIBUNG' : 'GUILD DESCRIPTION'}</div>
          {isOwner ? <>
            <textarea value={description} maxLength={500} rows={3} onChange={event => setDescription(event.target.value)} placeholder={de ? 'Ziele und Spielstil.' : 'Goals and play style.'} className="w-full resize-none rounded-xl border border-white/10 bg-black/45 px-3 py-2 text-[10px] leading-relaxed text-white outline-none placeholder:text-white/24 focus:border-amber-300/35" />
            <div className="flex items-center justify-between gap-3"><span className="text-[7px] text-white/24">{description.length}/500</span><ActionButton label={de ? 'Speichern' : 'Save'} onClick={saveDescription} disabled={busy} primary compact /></div>
          </> : <div className="text-[9px] leading-relaxed text-white/45">{membership.guild.description || (de ? 'Noch keine Beschreibung vorhanden.' : 'No description yet.')}</div>}
        </section>

        <details data-testid="guild-danger-zone" className="group rounded-2xl border border-red-400/14 bg-red-500/[.035]">
          <summary className="cursor-pointer list-none px-3 py-3 text-[7px] font-black uppercase tracking-[.18em] text-red-200/55"><span className="flex items-center justify-between"><span>{de ? 'MITGLIEDSCHAFT VERWALTEN' : 'MANAGE MEMBERSHIP'}</span><span className="transition-transform group-open:rotate-90">›</span></span></summary>
          <div className="space-y-2 border-t border-red-400/10 px-3 pb-3 pt-2">
            <div className="text-[9px] leading-relaxed text-white/38">{ownerMustTransfer
              ? (de ? 'Übertrage zuerst die Führung an ein anderes Mitglied.' : 'Transfer leadership to another member first.')
              : isOwner ? (de ? 'Beim Verlassen wird die Gilde vollständig aufgelöst.' : 'Leaving will disband the guild.') : (de ? 'Du kannst die Gilde jederzeit verlassen.' : 'You can leave at any time.')}</div>
            <ActionButton label={ownerMustTransfer ? (de ? 'Mitglied auswählen' : 'Choose member') : isOwner ? (de ? 'Gilde auflösen' : 'Disband guild') : (de ? 'Gilde verlassen' : 'Leave guild')} onClick={ownerMustTransfer ? () => setTab('members') : leaveCurrentGuild} disabled={busy} danger />
          </div>
        </details>
      </div>}
      ...
      {tab === 'members' && <section data-testid="guild-members-tab" className={`${scrollClass} space-y-2`}>
        {members.map(member => {
          ...
          return <div key={member.user_id} className="rounded-2xl border border-white/8 bg-white/[.025] p-3">
            <div className="flex items-start justify-between gap-3"><div className="min-w-0"><div className="truncate text-[11px] font-black text-white/82">{member.profile?.display_name ?? (de ? 'Unbekannter Spieler' : 'Unknown player')}</div><div className="mt-1 text-[7px] uppercase tracking-[.12em] text-amber-100/50">{roleLabel(member.role, de)}{member.joined_at ? ` · ${formatJoined(member.joined_at, language)}` : ''}</div></div><span className="rounded-full border border-amber-300/14 bg-amber-400/[.06] px-2 py-1 text-[6px] font-black uppercase text-amber-100/65">{member.role}</span></div>
            {isOwner && !ownerEntry && <div className="mt-3 grid grid-cols-3 gap-2"><ActionButton label={member.role === 'officer' ? (de ? 'Mitglied' : 'Member') : (de ? 'Offizier' : 'Officer')} onClick={() => changeRole(member, member.role === 'officer' ? 'member' : 'officer')} disabled={busy} compact /><ActionButton label={de ? 'Führung' : 'Leader'} onClick={() => transferLeadership(member)} disabled={busy} primary compact /><ActionButton label={de ? 'Entfernen' : 'Remove'} onClick={() => kickMember(member)} disabled={busy} danger compact /></div>}
          </div>;
        })}
      ...
      {tab === 'invite' && <section data-testid="guild-invite-tab" className={`${scrollClass} space-y-3`}>
        <div className="rounded-2xl border border-white/8 bg-white/[.025] p-3"><div className="text-[7px] font-black uppercase tracking-[.18em] text-white/35">{de ? 'SPIELER EINLADEN' : 'INVITE PLAYER'}</div><div className="mt-1 text-[9px] leading-relaxed text-white/34">{de ? 'Nutze den exakten Spielernamen aus dem Online-Profil.' : 'Use the exact player name from the online profile.'}</div></div>
        {canInvite ? <div className="space-y-2"><input value={inviteName} maxLength={24} onChange={event => setInviteName(event.target.value)} placeholder={de ? 'Spielername' : 'Player name'} className="w-full rounded-xl border border-white/10 bg-black/45 px-3 py-2.5 text-[11px] text-white outline-none placeholder:text-white/24 focus:border-amber-300/35" /><ActionButton label={de ? 'Einladung senden' : 'Send invitation'} onClick={sendInvite} disabled={busy} primary /></div> : <div className="rounded-xl border border-white/8 bg-black/25 p-3 text-[9px] text-white/38">{de ? 'Nur Anführer und Offiziere können Spieler einladen.' : 'Only leaders and officers can invite players.'}</div>}
      </section>}
    ...
    {session && !membership && <div className={`${scrollClass} space-y-3`}>
      <section className="space-y-3 rounded-2xl border border-amber-300/14 bg-amber-400/[.04] p-3">
        <div className="flex items-center justify-between gap-3 rounded-xl border border-white/8 bg-black/25 px-3 py-2.5"><span className="text-[7px] font-black uppercase text-white/34">{de ? 'DEIN GOLD' : 'YOUR GOLD'}</span><span className={`text-[11px] font-black ${canAffordGuild ? 'text-amber-100' : 'text-red-200'}`}>{formatGold(gold, language)} / {formatGold(GUILD_CREATION_COST, language)}</span></div>
        <input value={guildName} maxLength={32} onChange={event => setGuildName(event.target.value)} placeholder={de ? 'Gildenname' : 'Guild name'} className="w-full rounded-xl border border-white/10 bg-black/45 px-3 py-2.5 text-[11px] text-white outline-none placeholder:text-white/24" />
        <input value={guildTag} maxLength={6} onChange={event => setGuildTag(event.target.value.replace(/[^a-z0-9]/gi, '').toUpperCase())} placeholder={de ? 'Kürzel, z. B. VEIL' : 'Tag, e.g. VEIL'} className="w-full rounded-xl border border-white/10 bg-black/45 px-3 py-2.5 text-[11px] uppercase text-white outline-none placeholder:text-white/24" />
        <textarea value={guildDescription} maxLength={500} rows={3} onChange={event => setGuildDescription(event.target.value)} placeholder={de ? 'Beschreibung (optional)' : 'Description (optional)'} className="w-full resize-none rounded-xl border border-white/10 bg-black/45 px-3 py-2.5 text-[10px] text-white outline-none placeholder:text-white/24" />
        <ActionButton label={de ? `Für ${formatGold(GUILD_CREATION_COST, language)} Gold gründen` : `Create for ${formatGold(GUILD_CREATION_COST, language)} gold`} onClick={createNewGuild} disabled={busy || !canAffordGuild} primary />
      </section>
      {invites.length > 0 && <section className="space-y-2 rounded-2xl border border-white/8 bg-white/[.025] p-3"><div className="text-[7px] font-black uppercase tracking-[.18em] text-white/35">{de ? 'EINGEHENDE EINLADUNGEN' : 'INCOMING INVITATIONS'}</div>{invites.map(invite => <div key={invite.id} className="flex items-center gap-2 rounded-xl border border-white/8 bg-black/30 p-2.5"><div className="min-w-0 flex-1"><div className="truncate text-[10px] font-bold">[{invite.guild.tag}] {invite.guild.name}</div></div><ActionButton label="✓" onClick={() => answerInvite(invite.id, true)} disabled={busy} primary compact /><ActionButton label="×" onClick={() => answerInvite(invite.id, false)} disabled={busy} compact /></div>)}</section>}
      <ActionButton label={busy ? (de ? 'Lädt …' : 'Loading …') : (de ? 'Aktualisieren' : 'Refresh')} onClick={() => run(refreshGuildData)} disabled={busy} compact />
    </div>}
  </div>;
}
