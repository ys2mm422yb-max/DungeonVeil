...
export function ActionButtons({ gameState, onAttack, onDodge, onSkill, onInteract }: Props) {
  ...
  const Cooldown = ({ value }: { value: number }) => value > 0 ? (
    <div className="absolute inset-[9%] rounded-full pointer-events-none" style={{ background: `conic-gradient(rgba(0,0,0,.72) ${value * 360}deg, transparent 0deg)`, transform: 'rotate(-90deg)' }} />
  ) : null;
  ...
  return (
    <div className="fixed z-50 h-[154px] w-[164px] pointer-events-auto touch-none select-none" style={{ right: 'max(.55rem, env(safe-area-inset-right))', bottom: 'max(.7rem, calc(env(safe-area-inset-bottom) + .3rem))' }}>
