...
export function ActionButtons({ gameState: g, onDodge, variant = 'default' }: Props) {
  ...
  return (
    <div
      ...
      style={{
        width: worldBoss ? 72 : 68,
        height: worldBoss ? 72 : 68,
        right: 'max(16px,env(safe-area-inset-right))',
        bottom: worldBoss ? 'max(104px,calc(env(safe-area-inset-bottom) + 84px))' : 'max(22px,calc(env(safe-area-inset-bottom) + 14px))',
      }}
      data-ui-control
    >
      <button
      ...
      >
        ...
        <div className="relative z-10 flex flex-col items-center gap-0.5">
          <Wind size={worldBoss ? 20 : 18} strokeWidth={1.8} className={ready ? 'text-cyan-50' : 'text-white/35'} />
