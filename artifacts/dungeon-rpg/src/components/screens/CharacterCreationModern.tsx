...
export function CharacterCreationModern({onConfirm,onBack}:Props){
 ...
 return <div className="fixed left-0 top-0 z-50 flex h-[100dvh] w-full flex-col overflow-hidden bg-[#090807] text-amber-50" style={{touchAction:'auto'}}>
  ...
  <header className="relative z-10 flex shrink-0 items-center gap-3 px-5 pb-4 pt-6 pt-safe-top"><button onClick={onBack} className="h-11 w-11 rounded border border-amber-100/20 bg-black/35 text-2xl">‹</button><div><p className="text-[10px] tracking-[.35em] text-amber-200/45">DUNGEON VEIL</p><h1 className="font-serif text-2xl tracking-widest">{language==='de'?'DEIN SCHÜTZE':'YOUR ARCHER'}</h1></div></header>
