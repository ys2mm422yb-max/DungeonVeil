...
export function RunNamePromptScreen({ onConfirm, onBack }: { onConfirm: (name: string) => void | Promise<void>; onBack: () => void }) {
  ...
  return <div data-testid="run-name-prompt" className="fixed inset-0 z-[160] grid place-items-center bg-[radial-gradient(circle_at_50%_32%,rgba(151,100,34,.23),rgba(5,5,6,.98)_48%)] px-5 text-white">
    <section className="w-full max-w-sm rounded-[2rem] border border-amber-200/22 bg-black/72 p-5 shadow-[0_24px_80px_rgba(0,0,0,.72)] backdrop-blur-xl">
      <button type="button" onPointerDown={event => { event.preventDefault(); onBack(); }} aria-label={de ? 'Zurück' : 'Back'} className="grid h-10 w-10 place-items-center rounded-xl border border-white/12 bg-black/45 text-xl text-white/70 active:scale-90">‹</button>
      ...
      <button data-testid="run-name-confirm" type="button" disabled={!valid} onPointerDown={event => { event.preventDefault(); if (valid) void submit(); }} className="mt-3 w-full rounded-xl border border-amber-300/30 bg-amber-500/16 py-3 text-[9px] font-black uppercase tracking-[.18em] text-amber-100 active:scale-[.98] disabled:border-white/8 disabled:bg-white/[.03] disabled:text-white/22">{de ? 'RUN STARTEN' : 'START RUN'}</button>
    </section>
  </div>;
}
