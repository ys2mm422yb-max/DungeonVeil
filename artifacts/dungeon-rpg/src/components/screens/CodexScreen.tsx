...
const COPY = {
  locked: { de: 'NICHT ENTDECKT', en: 'UNDISCOVERED' },
  detailOpen: { de: 'DETAIL GEÖFFNET', en: 'DETAIL OPEN' },
  detailShow: { de: 'DETAIL ANZEIGEN', en: 'SHOW DETAILS' },
  silhouetteHint: { de: 'SILHOUETTE · FUNDHINWEIS', en: 'SILHOUETTE · DISCOVERY HINT' },
  type: { de: 'TYP', en: 'TYPE' },
  firstSighting: { de: 'ERSTER FUND', en: 'FIRST SIGHTING' },
  ...
  form: { de: 'GESTALT', en: 'FORM' },
  varies: { de: 'VARIIERT JE RUN', en: 'VARIES BY RUN' },
  rareHunt: { de: 'SELTENE JAGD', en: 'RARE HUNT' },
} satisfies Record<string, Copy>;

function pick(copy: Copy, de: boolean) { return de ? copy.de : copy.en; }

function CodexCard({ id, selected, known, title, eyebrow, lockedLabel, openLabel, openedLabel, hintLabel, artwork, onSelect }: CardProps) {
  return <button
    type="button"
...
function Fact({ label, value }: { label: string; value: string }) {
  return <div className="min-h-11 rounded-xl border border-white/7 bg-black/28 px-3 py-2.5">
...
export function CodexScreen({ onBack }: { onBack: () => void }) {
  ...
  const cardCopy = {
    lockedLabel: pick(COPY.locked, de), openLabel: pick(COPY.detailShow, de), openedLabel: pick(COPY.detailOpen, de), hintLabel: pick(COPY.silhouetteHint, de),
  };
  ...
  const cards = (() => {
    ...
    if (tab === 'hunts') return CODEX_HUNTS.map((entry, index) => {
      ...
      return <CodexCard {...cardCopy} key={entry.id} id={entry.id} selected={selected.hunts === entry.id} known={known}
    ...
    if (tab === 'wardens') return CODEX_WARDENS.map(entry => {
      ...
      return <CodexCard {...cardCopy} key={entry.id} id={entry.id} selected={selected.wardens === entry.id} known={known}
    ...
    if (tab === 'relics') return Object.values(VEIL_RELICS).map(relic => {
      ...
      return <CodexCard {...cardCopy} key={relic.id} id={relic.id} selected={selected.relics === relic.id} known={known}
    ...
    return equipment.map(item => {
      ...
      return <CodexCard {...cardCopy} key={item.id} id={item.id} selected={selected.equipment === item.id} known={known}
