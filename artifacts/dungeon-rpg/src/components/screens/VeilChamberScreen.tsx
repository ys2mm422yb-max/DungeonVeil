      ...
      <div className="relative mx-auto min-h-full max-w-md px-4 pb-[max(28px,calc(env(safe-area-inset-bottom)+18px))] pt-[max(24px,calc(env(safe-area-inset-top)+10px))]">
        ...
        <section className="mt-5 rounded-3xl border border-violet-300/15 bg-black/52 p-4 shadow-[0_22px_70px_rgba(0,0,0,.45)] backdrop-blur-xl">
          <div className="grid grid-cols-3 items-end gap-3">
            <div><div className="text-[8px] font-black tracking-[.24em] text-violet-200/45">{de ? 'SCHLEIER-RANG' : 'VEIL RANK'}</div><div className="mt-1 font-serif text-4xl text-white">{meta.rank}</div></div>
            <div className="text-center"><div className="text-[8px] font-black tracking-[.2em] text-yellow-200/45">GOLD</div><div className="mt-1 text-xl font-black text-yellow-200">{meta.gold}</div></div>
            <div className="text-right"><div className="text-[8px] font-black tracking-[.2em] text-amber-200/45">{de ? 'STAUB' : 'DUST'}</div><div className="mt-1 text-xl font-black text-amber-200">✦ {meta.dust}</div></div>
          </div>
        ...
        {tab === 'relic' ? (
          ...
          <>
            ...
            <section className="mt-3 grid gap-2">{items.map(item => { const progress = meta.owned[item.id]; const isEquipped = meta.equipped[item.slot] === item.id; const itemSource = SOURCE_LABELS[item.dropSource][de ? 'de' : 'en']; return <button key={item.id} type="button" onPointerDown={event => { event.preventDefault(); setSelected(item.id); }} className={`relative flex items-center gap-3 overflow-hidden rounded-2xl border p-3 text-left active:scale-[.99] ${selected === item.id ? 'border-white/22 bg-white/[.075]' : 'border-white/8 bg-black/38'}`}><div className="h-3 w-3 shrink-0 rounded-full shadow-[0_0_16px_currentColor]" style={{ color: item.accent, background: item.accent }} /><div className="min-w-0 flex-1"><div className="truncate text-[12px] font-black text-white/82">{de ? item.nameDe : item.nameEn}</div><div className="mt-1 text-[8px] uppercase tracking-[.14em] text-white/30">{progress ? `${de ? 'STUFE' : 'LEVEL'} ${progress.level} · ${progress.copies} ${de ? 'KOPIEN' : 'COPIES'}` : meta.rank >= item.unlockRank ? `${de ? 'NOCH NICHT GEFUNDEN' : 'NOT FOUND'} · ${itemSource}` : `${de ? 'AB RANG' : 'FROM RANK'} ${item.unlockRank} · ${itemSource}`}</div></div>{isEquipped && <span className="rounded-full border border-emerald-300/20 bg-emerald-400/10 px-2 py-1 text-[7px] font-black tracking-[.12em] text-emerald-200">{de ? 'AKTIV' : 'ACTIVE'}</span>}</button>; })}</section>
          </>
        ) : null}
      </div>
    </div>
