...
export function VeilChamberScreen({ onBack }: { onBack: () => void }) {
  ...
  const isEquipped = isOwned && meta.equipped[selectedItem.slot] === selected;
  const upgradeCost = isOwned ? equipmentUpgradeCost(selected, meta) : 0;
  ...
  const openSlot = (slot: EquipmentSlot) => { setTab('gear'); selectItem(meta.equipped[slot]); };
    ...
    <div className="relative mx-auto min-h-full max-w-md px-4 pb-[max(30px,calc(env(safe-area-inset-bottom)+18px))] pt-[max(22px,calc(env(safe-area-inset-top)+8px))]">
      ...
      {tab === 'gear' ? <>
        ...
        <section className="mt-4 overflow-hidden rounded-3xl border border-white/10 bg-black/50 shadow-[0_22px_70px_rgba(0,0,0,.45)]">
          {isOwned ? <div className="grid min-h-[240px] grid-cols-[44%_56%]">
          ...
          </div> : <div className="flex min-h-[240px] flex-col items-center justify-center px-8 text-center">
        ...
        <section className="mt-4">
          <div className="mb-3 flex items-center justify-between"><div className="text-[8px] font-black uppercase tracking-[.25em] text-white/35">{de ? 'GEGENSTÄNDE' : 'ITEMS'}</div><div className="text-[8px] font-black tracking-[.12em] text-white/25">{discoveredCount}/{gear.length}</div></div>
          <div className="grid grid-cols-3 gap-2">
            {gear.map(item => {
              ...
              return <button key={item.id} type="button" onPointerDown={event => { event.preventDefault(); selectItem(item.id); }} className={`relative min-h-[96px] rounded-2xl border p-3 text-left active:scale-[.98] ${selected === item.id ? 'border-amber-300/45 bg-amber-400/10' : 'border-white/8 bg-black/42'} ${owned ? '' : 'bg-[linear-gradient(145deg,rgba(255,255,255,.018),rgba(0,0,0,.62))]'}`}>
        ...
        <section className="mt-4 rounded-3xl border border-violet-300/18 bg-black/48 p-4">
          ...
          {selectedRelicDef ? <><div className="mt-4 flex items-center gap-4"><div className="grid h-16 w-16 shrink-0 place-items-center rounded-full border border-white/12 bg-black/35 text-3xl shadow-[0_0_30px_currentColor]" style={{ color: selectedRelicDef.accent }}>◆</div><div><h2 className="text-xl font-black text-white">{de ? selectedRelicDef.nameDe : selectedRelicDef.nameEn}</h2><div className="mt-1 text-[8px] font-black uppercase tracking-[.14em] text-white/30">{selectedRelicDef.source === 'hunt' ? (de ? 'JAGD-DROP' : 'HUNT DROP') : (de ? 'WÄCHTER-DROP' : 'WARDEN DROP')}</div></div></div><p className="mt-4 text-[12px] leading-relaxed text-white/65">{de ? selectedRelicDef.descriptionDe : selectedRelicDef.descriptionEn}</p><button type="button" onPointerDown={event => { event.preventDefault(); const next = equipVeilRelic(selectedRelicDef.id); setRelicProfile({ ...next }); }} disabled={relicProfile.equipped === selectedRelicDef.id} className="mt-4 w-full rounded-xl border border-violet-300/25 bg-violet-500/14 py-3 text-[9px] font-black uppercase tracking-[.14em] text-violet-100 disabled:border-emerald-300/20 disabled:bg-emerald-400/10 disabled:text-emerald-200/55 active:scale-[.98]">{relicProfile.equipped === selectedRelicDef.id ? (de ? 'AKTIV' : 'EQUIPPED') : (de ? 'AUSRÜSTEN' : 'EQUIP')}</button></> : <div className="mt-5 rounded-2xl border border-white/8 bg-white/[.025] px-4 py-8 text-center text-[10px] font-bold leading-relaxed text-white/35">{de ? 'Noch kein seltenes Relikt gefunden.' : 'No rare relic found yet.'}</div>}
        </section>
        <section className="mt-4 grid grid-cols-2 gap-2">
          {Object.values(VEIL_RELICS).map(relic => { const owned = relicProfile.owned.includes(relic.id); const active = relicProfile.equipped === relic.id; return <button key={relic.id} type="button" disabled={!owned} onPointerDown={event => { event.preventDefault(); if (owned) setSelectedRelic(relic.id); }} className={`relative min-h-[105px] rounded-2xl border p-3 text-left active:scale-[.98] ${selectedRelic === relic.id && owned ? 'border-violet-300/40 bg-violet-400/10' : 'border-white/8 bg-black/42'} ${owned ? '' : 'opacity-35'}`}><div className="h-3 w-3 rounded-full shadow-[0_0_14px_currentColor]" style={{ color: owned ? relic.accent : '#777', background: owned ? relic.accent : '#333' }} /><div className="mt-3 text-[11px] font-black leading-tight text-white/82">{owned ? (de ? relic.nameDe : relic.nameEn) : '???'}</div><div className="mt-2 text-[7px] font-black uppercase tracking-[.12em] text-white/28">{owned ? (de ? 'GEFUNDEN' : 'FOUND') : (de ? 'UNENTDECKT' : 'UNDISCOVERED')}</div>{active && <span className="absolute right-2 top-2 h-2 w-2 rounded-full bg-emerald-300 shadow-[0_0_10px_rgba(110,255,180,.8)]" />}</button>; })}
        </section>
      </>}
    </div>
  </div>;
