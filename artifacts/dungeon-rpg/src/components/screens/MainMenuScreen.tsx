...
export function MainMenuScreen(props: Props) {
  ...
  const action = (label: string, detail: string, icon: IconName, onClick: () => void, tone: 'gold' | 'violet' | 'dark' | 'blue', disabled = false) => {
    ...
    return <button type="button" disabled={disabled} onClick={event => { event.preventDefault(); event.stopPropagation(); if (!disabled) onClick(); }} className={`group min-h-[54px] w-full rounded-[16px] border px-2.5 py-2 text-left shadow-[0_10px_22px_rgba(0,0,0,.3)] backdrop-blur-md transition active:scale-[.975] ${toneClass} ${disabled ? 'opacity-35' : ''}`}>
  ...
  return <div className="fixed inset-0 z-50 select-none overflow-hidden bg-[#050308] text-white">
    ...
    {overlay && overlay !== 'profile' && overlay !== 'more' && <div className="absolute inset-0 z-40 flex items-center justify-center bg-[#06070b]/78 px-3 py-[max(12px,env(safe-area-inset-top))] backdrop-blur-md md:px-6" onClick={() => setOverlay(null)}><div className="w-full max-w-sm" onClick={event => event.stopPropagation()}>
      ...
      {overlay !== 'guild' && <button type="button" onClick={event => { event.preventDefault(); event.stopPropagation(); setOverlay(null); }} className="mt-3 w-full rounded-2xl border border-amber-50/14 bg-[#11100f]/88 py-3 text-[9px] font-black uppercase tracking-[.2em] text-amber-50/58">{language === 'de' ? 'SCHLIESSEN' : 'CLOSE'}</button>}
    </div></div>}
  </div>;
}
