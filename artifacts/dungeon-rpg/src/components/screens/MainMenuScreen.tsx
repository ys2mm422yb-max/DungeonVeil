  ...
  return <div className="fixed inset-0 z-50 select-none overflow-hidden bg-[#050308] text-white">
    ...
    <div className="absolute right-3 top-[max(10px,calc(env(safe-area-inset-top)+4px))] z-30 flex items-start gap-1.5">
      <div className="space-y-1">
        ...
        <button data-testid="main-menu-gold-button" type="button" aria-label={language === 'de' ? 'Mehr' : 'More'} aria-expanded={overlay === 'more'} onPointerDown={event => { event.preventDefault(); setOverlay(current => current === 'more' ? null : 'more'); }} className="flex h-7 min-w-[92px] items-center rounded-[11px] border border-amber-200/12 bg-black/58 px-2 text-[9px] font-black text-white/72 backdrop-blur-xl active:scale-95"><MenuIcon name="coin" className="mr-1.5 h-3.5 w-3.5 text-amber-400" /><span className="flex-1 text-left">{gold}</span><span className="text-sm text-white/32">＋</span></button>
      </div>
    ...
    {overlay === 'more' && <div data-testid="gold-menu-backdrop" className="absolute inset-0 z-40 bg-black/18" onPointerDown={() => setOverlay(null)}>
      <div data-testid="gold-menu-popover" className="absolute right-3 top-[max(76px,calc(env(safe-area-inset-top)+68px))] w-[min(330px,calc(100vw-24px))]" onPointerDown={event => event.stopPropagation()}>
        <div className="rounded-3xl border border-amber-50/16 bg-[#17130f]/98 p-3.5 shadow-[0_24px_70px_rgba(0,0,0,.62)] backdrop-blur-xl">
          <div className="mb-3 flex items-center justify-between gap-3 px-1"><div><div className="text-[7px] font-black uppercase tracking-[.25em] text-amber-50/42">{language === 'de' ? 'WEITERE OPTIONEN' : 'MORE OPTIONS'}</div><div className="mt-1 text-[10px] font-black text-amber-100/75">{gold} Gold</div></div><button type="button" aria-label={language === 'de' ? 'Schließen' : 'Close'} onPointerDown={event => { event.preventDefault(); setOverlay(null); }} className="grid h-8 w-8 place-items-center rounded-xl border border-white/10 bg-black/30 text-lg text-white/55">×</button></div>
    ...
    {overlay && overlay !== 'profile' && overlay !== 'more' && <div className="absolute inset-0 z-40 flex items-center justify-center bg-[#06070b]/78 px-3 py-[max(12px,env(safe-area-inset-top))] backdrop-blur-md md:px-6" onPointerDown={() => setOverlay(null)}><div className="w-full max-w-sm" onPointerDown={event => event.stopPropagation()}>
      ...
      {overlay === 'mailbox' && <MailboxPanel language={language} onUnreadChange={setMailUnread} onOpenOnline={() => setOverlay('online')} />}
