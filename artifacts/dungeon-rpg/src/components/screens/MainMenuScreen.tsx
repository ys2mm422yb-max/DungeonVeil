...
export function MainMenuScreen(props: Props) {
  ...
  const button = (label: string, action: () => void, subtitle?: string, kind: 'normal' | 'primary' | 'chamber' = 'normal', disabled = false) => {
    ...
    return (
      <button type="button" disabled={disabled} onPointerDown={event => { event.preventDefault(); if (!disabled) action(); }} className={`w-full rounded-[1.35rem] border px-5 py-3.5 text-left backdrop-blur-xl active:scale-[.975] ${kindClass} ${disabled ? 'opacity-35' : ''}`}>
        <div className="flex items-center gap-4">
          <div className="min-w-0 flex-1">
            <div className="text-[15px] font-black tracking-[.13em]">{label}</div>
            {subtitle && <div className="mt-1 truncate text-[8px] uppercase tracking-[.14em] text-white/35">{subtitle}</div>}
          </div>
          {!disabled && <span className="text-lg opacity-38">›</span>}
        </div>
      </button>
    );
  };
    ...
    <div className="fixed inset-0 z-50 select-none overflow-hidden bg-[#070706] text-white">
      ...
      <div className="relative flex h-full flex-col px-5 pb-[max(22px,calc(env(safe-area-inset-bottom)+8px))] pt-[max(34px,calc(env(safe-area-inset-top)+14px))]">
        <header className="text-center">
          <div className="text-[7px] font-black uppercase tracking-[.5em] text-amber-200/42">{language === 'de' ? 'BETRITT DEN SCHLEIER' : 'ENTER THE VEIL'}</div>
          <h1 className="mt-1.5 font-serif text-[clamp(2.45rem,10.5vw,3.7rem)] font-black leading-[.86] tracking-[.07em] text-[#d7a347]">DUNGEON<br />VEIL</h1>
          <p className="mt-2.5 text-[7px] uppercase tracking-[.28em] text-white/28">{t.subtitle}</p>
        </header>

        <div className="mt-5 flex justify-center gap-3">
          <button type="button" onPointerDown={event => { event.preventDefault(); setOverlay('daily'); }} className="relative grid h-11 w-11 place-items-center rounded-full border border-amber-300/22 bg-black/58 text-base text-amber-200 backdrop-blur-xl active:scale-95">
            ✦
            <span className="absolute -right-1 -top-1 rounded-full border border-amber-100/25 bg-amber-500 px-1.5 py-0.5 text-[7px] font-black text-black">{retention.daily.claimed.length}/3</span>
          </button>
      ...
      {overlay && <div className="absolute inset-0 z-40 flex items-center justify-center bg-black/74 px-5 backdrop-blur-md" onPointerDown={() => setOverlay(null)}>
        <div className="w-full max-w-sm" onPointerDown={event => event.stopPropagation()}>
          {overlay === 'daily' && <DailyQuestPanel />}
          {overlay === 'rift' && <WeeklyRiftPanel language={language} />}
          {overlay === 'more' && <div className="rounded-3xl border border-white/10 bg-[#0c0b0a]/95 p-4 shadow-2xl">
            <div className="mb-3 px-2 text-[8px] font-black uppercase tracking-[.25em] text-white/32">{language === 'de' ? 'WEITERE OPTIONEN' : 'MORE OPTIONS'}</div>
            <div className="space-y-2">
              {button(t.settings, () => { setOverlay(null); props.onSettings(); })}
              {button(t.credits, () => { setOverlay(null); props.onCredits(); })}
            </div>
          </div>}
