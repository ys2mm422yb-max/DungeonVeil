  ...
  return <div className="fixed inset-0 z-50 select-none overflow-hidden bg-[#050308] text-white">
    ...
    <div className="absolute right-3 top-[max(10px,calc(env(safe-area-inset-top)+4px))] z-30 flex items-start gap-1.5">
      <div className="space-y-1">
        <button data-testid="main-menu-dust-button" type="button" onClick={event => { event.preventDefault(); event.stopPropagation(); armMobilePointerSafety(); props.onVeilChamber(); }} className="flex h-7 min-w-[92px] items-center rounded-[11px] border border-violet-300/14 bg-black/58 px-2 text-[9px] font-black text-white/72 backdrop-blur-xl active:scale-95"><MenuIcon name="dust" className="mr-1.5 h-3.5 w-3.5 text-violet-400" /><span className="flex-1 text-left">{Number(meta.dust ?? 0).toLocaleString(language === 'de' ? 'de-DE' : 'en-US')}</span><span className="text-sm text-white/32">＋</span></button>
        <button data-testid="main-menu-gold-button" type="button" aria-label={language === 'de' ? 'Gold und weitere Optionen' : 'Gold and more options'} aria-expanded={overlay === 'more'} onClick={event => { event.preventDefault(); event.stopPropagation(); armMobilePointerSafety(); setOverlay(current => current === 'more' ? null : 'more'); }} className={`flex h-7 min-w-[92px] items-center rounded-[11px] border px-2 text-[9px] font-black backdrop-blur-xl active:scale-95 ${overlay === 'more' ? 'border-amber-200/30 bg-amber-500/14 text-amber-50' : 'border-amber-200/12 bg-black/58 text-white/72'}`}><MenuIcon name="coin" className="mr-1.5 h-3.5 w-3.5 text-amber-400" /><span className="flex-1 text-left">{gold}</span><span className="text-sm text-white/32">＋</span></button>
      </div>
      <button type="button" aria-label={language === 'de' ? 'Optionen' : 'Options'} onClick={event => { event.preventDefault(); event.stopPropagation(); armMobilePointerSafety(); props.onSettings(); }} className="grid h-[60px] w-10 place-items-center rounded-[15px] border border-white/[.09] bg-black/58 text-white/60 backdrop-blur-xl active:scale-95"><MenuIcon name="settings" className="h-5 w-5" /></button>
    </div>
