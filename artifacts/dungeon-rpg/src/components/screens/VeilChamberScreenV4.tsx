...
export function VeilChamberScreen({ onBack }: { onBack: () => void }) {
  ...
  return <div className="fixed inset-0 z-[70] overflow-y-auto bg-[#080706] text-white">
    <div className={'relative mx-auto min-h-full px-4 pb-8 pt-[max(24px,calc(env(safe-area-inset-top)+10px))] ' + (tab === 'relic' ? 'max-w-5xl' : 'max-w-md')}>
      ...
      {tab === 'relic' ? <>
        <section data-testid="relic-source-summary" className="mt-4 rounded-2xl border border-violet-300/15 bg-violet-500/[.06] p-3 text-violet-100/55">
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            <p className="text-[8px] leading-relaxed">{de ? 'Jagd-Relikte kommen aus Jagden. Boss-Relikte können in den Bossräumen 10, 20, 30, 40 und 50 fallen. Der Weltenkern kommt ausschließlich vom Weltboss.' : 'Hunt relics come from hunts. Boss relics can drop in boss rooms 10, 20, 30, 40 and 50. The World Core is exclusive to the world boss.'}</p>
            <span data-testid="relic-collection-count" className="shrink-0 rounded-full border border-violet-200/15 bg-black/35 px-3 py-1.5 text-[8px] font-black tracking-[.16em] text-violet-100/70">{relics.owned.length} / {Object.keys(VEIL_RELICS).length} {de ? 'GEBORGEN' : 'RECOVERED'}</span>
          </div>
        </section>
          ...
          <section data-testid="relic-detail-panel" className="rounded-3xl border border-violet-300/25 bg-black/52 p-5 md:sticky md:top-4">
            {activeRelic ? <>
              <div className="flex items-center justify-between gap-3">
                <span className="rounded-full border border-white/10 bg-white/[.04] px-3 py-1 text-[7px] font-black tracking-[.16em]" style={{ color: activeRelic.accent }}>{RELIC_SOURCE_LABELS[activeRelic.source][de ? 'de' : 'en']}</span>
                {relics.equipped === activeRelic.id && <span data-testid="relic-active-badge" className="rounded-full border border-emerald-300/25 bg-emerald-300/10 px-3 py-1 text-[7px] font-black tracking-[.16em] text-emerald-100">{de ? 'AKTIV' : 'ACTIVE'}</span>}
              </div>
              <div className="mx-auto mt-6 flex h-28 w-28 items-center justify-center rounded-full border border-white/10 bg-black/40 shadow-[0_0_42px_rgba(142,92,255,.14)]" style={{ boxShadow: '0 0 44px ' + activeRelic.accent + '24' }}>
                <span aria-hidden="true" className="text-6xl leading-none" style={{ color: activeRelic.accent }}>{activeRelic.source === 'hunt' ? '◈' : activeRelic.source === 'boss' ? '♛' : '✦'}</span>
              </div>
              ...
              <p className="mt-2 max-w-[250px] text-[9px] leading-relaxed text-white/25">{de ? 'Besiege Jagdgegner, Kapitelbosse und den Weltboss, um die Sammlung zu öffnen.' : 'Defeat hunt enemies, chapter bosses and the world boss to open the collection.'}</p>
            </div>}
          </section>
          <section data-testid="relic-card-grid" className="grid content-start gap-2 sm:grid-cols-2">
            {Object.values(VEIL_RELICS).map(relic => {
              const owned = relics.owned.includes(relic.id);
              const active = relics.equipped === relic.id;
              const selectedCard = selectedRelic === relic.id;
              const sourceLabel = RELIC_SOURCE_LABELS[relic.source][de ? 'de' : 'en'];
              return <button
                key={relic.id}
                data-testid={'relic-card-' + relic.id}
                data-relic-card="true"
                data-owned={owned ? 'true' : 'false'}
                data-active={active ? 'true' : 'false'}
                disabled={!owned}
                onPointerDown={event => { event.preventDefault(); if (owned) { setSelectedRelic(relic.id); markRelicSeen(relic.id); } }}
                className={'relative min-h-[174px] overflow-hidden rounded-3xl border p-4 text-left transition ' + (selectedCard ? 'border-violet-200/45 bg-violet-400/[.11]' : active ? 'border-emerald-300/28 bg-emerald-300/[.06]' : owned ? 'border-white/10 bg-black/42 active:scale-[.99]' : 'cursor-not-allowed border-white/6 bg-black/28')}
              >
                <span className="pointer-events-none absolute -right-8 -top-10 h-28 w-28 rounded-full blur-2xl" style={{ background: owned ? relic.accent + '1f' : 'rgba(255,255,255,.025)' }} />
                <div className="relative flex items-center justify-between gap-2">
                  <span className="rounded-full border border-white/8 bg-black/25 px-2.5 py-1 text-[6px] font-black tracking-[.12em]" style={{ color: owned ? relic.accent : 'rgba(255,255,255,.22)' }}>{sourceLabel}</span>
                  {active && <span className="rounded-full bg-emerald-300/12 px-2 py-1 text-[6px] font-black text-emerald-100">{de ? 'AKTIV' : 'ACTIVE'}</span>}
                </div>
                <div className="relative mt-4 flex items-start gap-3">
                  <span aria-hidden="true" data-testid={owned ? undefined : 'relic-locked-silhouette'} className={'flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl border text-3xl ' + (owned ? 'border-white/10 bg-black/35' : 'border-dashed border-white/7 bg-white/[.02] text-white/10 grayscale')} style={owned ? { color: relic.accent } : undefined}>{relic.source === 'hunt' ? '◈' : relic.source === 'boss' ? '♛' : '✦'}</span>
                  <span className="min-w-0 flex-1">
                    <b className={'block text-[12px] leading-snug ' + (owned ? 'text-white' : 'text-white/24')}>{owned ? (de ? relic.nameDe : relic.nameEn) : (de ? 'UNENTDECKTES RELIKT' : 'UNDISCOVERED RELIC')}</b>
                    <small className={'mt-2 block text-[8px] leading-relaxed ' + (owned ? 'text-white/42' : 'text-white/20')}>{owned ? (de ? relic.descriptionDe : relic.descriptionEn) : (de ? 'Silhouette bekannt · Effekt wird nach der Bergung sichtbar.' : 'Silhouette known · effect is revealed after recovery.')}</small>
                  </span>
                </div>
                <div className="relative mt-4 flex items-center justify-between text-[6px] font-black tracking-[.13em]">
                  <span className={owned ? 'text-violet-100/45' : 'text-white/16'}>{owned ? (selectedCard ? (de ? 'AUSGEWÄHLT' : 'SELECTED') : (de ? 'DETAILS ÖFFNEN' : 'OPEN DETAILS')) : (de ? 'GESPERRT' : 'LOCKED')}</span>
                  {newRelics.has(relic.id) && <span data-testid="inventory-item-new-badge" className="rounded-full bg-red-500 px-2 py-1 text-[6px] text-white">{de ? 'NEU' : 'NEW'}</span>}
                </div>
              </button>;
            })}
          </section>
