...
import { equipVeilRelic, loadVeilRelicProfile, type VeilRelicId } from '../../game/veilRelics';
import {
...
import { RelicInventoryPanelV4 } from '../RelicInventoryPanelV4';
...
export function VeilChamberScreen({ onBack }: { onBack: () => void }) {
  ...
  const selectItem = (id: EquipmentId) => {
    setSelected(id);
    markEquipmentSeen(id);
  };
  const selectRelic = (id: VeilRelicId) => {
    setSelectedRelic(id);
    if (relics.owned.includes(id)) markRelicSeen(id);
  };
  const equipRelic = (id: VeilRelicId) => {
    setRelics({ ...equipVeilRelic(id) });
  };
  const upgrade = () => {
    ...
    setMeta({ ...next });
    if ((next.owned[item.id]?.level ?? before) > before) {
      window.dispatchEvent(new CustomEvent('dungeon-veil-retention-toast', {
        detail: {
          title: de ? 'ITEM VERBESSERT' : 'ITEM UPGRADED',
          text: `${de ? item.nameDe : item.nameEn} · LEVEL ${next.owned[item.id]?.level}/5`,
          tone: 'relic',
        },
      }));
    }
  ...
  const craft = () => {
    ...
    if (result.crafted) {
      window.dispatchEvent(new CustomEvent('dungeon-veil-retention-toast', {
        detail: {
          title: result.newUnlock
            ? (de ? 'ITEM HERGESTELLT' : 'ITEM CRAFTED')
            : (de ? 'KOPIE HERGESTELLT' : 'COPY CRAFTED'),
          text: de ? item.nameDe : item.nameEn,
          tone: 'relic',
        },
      }));
    }
  ...
  return <div className="fixed inset-0 z-[70] overflow-y-auto bg-[#080706] text-white">
    <div data-testid="inventory-responsive-shell" className={`relative mx-auto min-h-full px-4 pb-8 pt-[max(24px,calc(env(safe-area-inset-top)+10px))] md:px-6 ${tab === 'relic' ? 'max-w-6xl' : 'max-w-md'}`}>
      <header className="flex gap-3">
        <button type="button" onPointerDown={event => { event.preventDefault(); onBack(); }} className="h-12 w-12 rounded-xl border border-white/12 bg-black/45 text-2xl">‹</button>
        <div>
          <div className="text-[8px] font-black tracking-[.42em] text-violet-200/45">DUNGEON VEIL</div>
          <h1 className="font-serif text-3xl text-[#e7c37a]">{de ? 'INVENTAR' : 'INVENTORY'}</h1>
          <p className="text-[8px] tracking-[.16em] text-white/35">{de ? '10 GAMEPLAY-ITEMS · 7 RELIKTE SEPARAT' : '10 GAMEPLAY ITEMS · 7 RELICS SEPARATE'}</p>
        </div>
      </header>

      <section className={`mt-5 rounded-3xl border border-violet-300/15 bg-black/52 p-4 ${tab === 'relic' ? 'md:grid md:grid-cols-[minmax(0,1fr)_minmax(340px,.6fr)] md:items-center md:gap-5' : ''}`}>
        <div className="grid grid-cols-3 gap-3 text-center">
          <div><div className="text-[8px] text-violet-200/45">{de ? 'RANG' : 'RANK'}</div><div className="text-3xl">{meta.rank}</div></div>
          <div><div className="text-[8px] text-yellow-200/45">GOLD</div><div className="text-lg text-yellow-200">{meta.gold}</div></div>
          <div><div className="text-[8px] text-amber-200/45">{de ? 'STAUB' : 'DUST'}</div><div className="text-lg text-amber-200">✦ {meta.dust}</div></div>
        </div>
        <div>
          <div className="mt-3 h-2 rounded-full bg-white/7 md:mt-0"><div className="h-full rounded-full bg-violet-400" style={{ width: `${xpPercent}%` }} /></div>
          <p data-testid="equipment-permanent-progression-copy" className="mt-3 text-center text-[8px] leading-relaxed text-white/38">{de ? 'Ausrüstungslevel 1–5 sind dauerhaft. Relikte bleiben eine eigene Sammlung und verbrauchen keinen Ausrüstungsplatz.' : 'Equipment levels 1–5 are permanent. Relics remain a separate collection and consume no equipment slot.'}</p>
        </div>
      </section>

      <div className="mt-4 grid grid-cols-4 gap-1.5">
        {TABS.map(key => {
          const count = key === 'relic' ? newRelics.size : [...newEquipment].filter(id => EQUIPMENT[id].slot === key).length;
          return <button data-testid={`inventory-tab-${key}`} key={key} type="button" onPointerDown={event => { event.preventDefault(); changeTab(key); }} className={`relative rounded-xl border py-3 text-[7px] font-black ${tab === key ? 'border-amber-300/45 bg-amber-400/12 text-amber-100' : 'border-white/9 bg-black/42 text-white/38'}`}>
            {key === 'relic' ? (de ? 'RELIKT' : 'RELIC') : SLOT_LABELS[key][de ? 'de' : 'en']}
            {count > 0 && <span data-testid="inventory-tab-new-badge" className="absolute -right-1 -top-1 rounded-full bg-red-500 px-1.5 py-0.5 text-[6px] text-white">{de ? 'NEU' : 'NEW'} {count}</span>}
          </button>;
        })}
      </div>

      {tab === 'relic' ? <RelicInventoryPanelV4
        profile={relics}
        selectedId={selectedRelic}
        language={language}
        newRelics={newRelics}
      ...
      /> : item ? <>
        <section className="mt-4 overflow-hidden rounded-3xl border border-white/10 bg-black/52">
          <div className="grid min-h-[245px] grid-cols-[42%_58%]">
            ...
            <div className="flex flex-col p-4">
              <div className="text-[8px] font-black" style={{ color: item.accent }}>{SLOT_LABELS[item.slot as 'bow' | 'quiver' | 'armor'][de ? 'de' : 'en']}</div>
              ...
              {level > 0 && <div className="mt-4 grid gap-2">
                {cost ? <div data-testid="equipment-upgrade-preview" className="rounded-xl border border-amber-300/14 bg-amber-300/[.05] p-2.5">
                  <div className="text-[7px] font-black text-amber-100/60">LEVEL {level} → {level + 1}</div>
                  <div className="mt-2 grid gap-1">{preview.map(row => <div key={row.key} className="flex justify-between rounded-lg bg-black/25 px-2 py-1.5 text-[8px]"><span>{VALUE_LABELS[row.key][de ? 'de' : 'en']}</span><span>{shown(row.current, row.format === 'percent')} → {shown(row.next, row.format === 'percent')} <b className="text-emerald-200">(+{shown(row.delta, row.format === 'percent')})</b></span></div>)}</div>
                </div> : <div data-testid="equipment-upgrade-max" className="rounded-xl border border-emerald-300/14 p-3 text-center text-[8px] text-emerald-200">MAXIMALLEVEL</div>}
                <div className="grid grid-cols-2 gap-2">
                  <button type="button" disabled={meta.equipped[item.slot] === item.id} onPointerDown={event => { event.preventDefault(); setMeta({ ...equipMetaItem(item.id) }); }} className="rounded-xl border border-violet-300/25 bg-violet-500/14 py-3 text-[9px] font-black disabled:opacity-35">{meta.equipped[item.slot] === item.id ? (de ? 'AKTIV' : 'ACTIVE') : (de ? 'AUSRÜSTEN' : 'EQUIP')}</button>
                  <button data-testid="equipment-upgrade-button" type="button" disabled={!canUpgrade} onClick={event => { event.preventDefault(); event.stopPropagation(); upgrade(); }} className="rounded-xl border border-amber-300/25 bg-amber-500/12 py-3 text-[9px] font-black disabled:opacity-30">{upgrading ? '…' : cost ? (de ? 'VERBESSERN' : 'UPGRADE') : 'MAX'}</button>
                </div>
        ...
        {targetable && <section className="mt-3 rounded-2xl border border-violet-300/12 bg-black/42 p-3">
          <div data-testid="equipment-source-marks" className="flex justify-between text-[8px]"><span>{SOURCE_LABELS[item.dropSource][de ? 'de' : 'en']}-{de ? 'MARKEN' : 'MARKS'}</span><span>{sourceMarks}/{markCost}</span></div>
          <div className="mt-2 grid grid-cols-2 gap-2">
            <button data-testid="equipment-wish-item" type="button" onPointerDown={event => { event.preventDefault(); setTargeting({ ...setEquipmentWishItem(targeting.wishItem === item.id ? null : item.id) }); }} className="rounded-xl border border-fuchsia-300/25 py-3 text-[8px] font-black">{targeting.wishItem === item.id ? (de ? '◎ WUNSCH AKTIV' : '◎ WISH ACTIVE') : (de ? '◎ ALS WUNSCH' : '◎ SET WISH')}</button>
            <button data-testid="equipment-craft-copy" type="button" disabled={!canCraft} onPointerDown={event => { event.preventDefault(); craft(); }} className="rounded-xl border border-emerald-300/25 py-3 text-[8px] font-black disabled:opacity-30">{level ? (de ? '⚒ KOPIE BAUEN' : '⚒ CRAFT COPY') : (de ? '⚒ ITEM BAUEN' : '⚒ CRAFT ITEM')}</button>
          </div>
          <p className="mt-2 text-center text-[7px] text-white/30">{de ? 'Wunschchance 18–24 % · Hard-Pity nach 7–9 passenden Fehlschlägen.' : 'Wish chance 18–24% · hard pity after 7–9 matching misses.'}</p>
        </section>}

        <section className="mt-3 grid gap-2">{items.map(entry => {
          const owned = meta.owned[entry.id];
          return <button key={entry.id} type="button" onPointerDown={event => { event.preventDefault(); selectItem(entry.id); }} className={`flex items-center gap-3 rounded-2xl border p-3 text-left ${selected === entry.id ? 'border-white/22 bg-white/[.075]' : 'border-white/8 bg-black/38'}`}>
            <span className="h-3 w-3 rounded-full" style={{ background: entry.accent }} />
            <span className="min-w-0 flex-1"><b className="block truncate text-[12px]">{de ? entry.nameDe : entry.nameEn}</b><small className="text-[8px] text-white/30">{owned ? `LEVEL ${owned.level} · ${owned.copies} ${de ? 'KOPIEN' : 'COPIES'}` : `${de ? 'KAPITEL' : 'CHAPTER'} ${equipmentUnlockChapter(entry.id)} · ${SOURCE_LABELS[entry.dropSource][de ? 'de' : 'en']}`}</small></span>
            {newEquipment.has(entry.id) && <span data-testid="inventory-item-new-badge" className="rounded-full bg-red-500 px-2 py-1 text-[6px]">{de ? 'NEU' : 'NEW'}</span>}
          </button>;
        })}</section>
      </> : null}
    </div>
