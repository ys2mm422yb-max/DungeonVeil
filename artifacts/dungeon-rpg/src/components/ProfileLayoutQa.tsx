...
function initialView(): View { return typeof window !== 'undefined' && new URLSearchParams(window.location.search).get('profile') === 'public' ? 'public' : 'own'; }
function captureMode() { return typeof window !== 'undefined' && new URLSearchParams(window.location.search).get('capture') === '1'; }

function qaMeta(): MetaProgression {
  const meta = loadMetaProgression();
  meta.rank = 12;
  meta.owned = {
    'ash-bow': { level: 2, copies: 1 }, 'ember-bow': { level: 3, copies: 2 }, 'ranger-quiver': { level: 2, copies: 1 },
    'black-quiver': { level: 2, copies: 0 }, 'ranger-cloak': { level: 3, copies: 2 }, 'ash-armor': { level: 4, copies: 1 },
  };
...
function qaProfile(): PlayerProfileProgress {
  ...
  profile.stats = { runsStarted: 18, roomsCleared: 146, enemiesDefeated: 904, bossesDefeated: 21, totalDamage: 187450, itemsFound: 14, questsCompleted: 27, playTimeMs: 8_640_000, highestChapter: 7, highestRoom: 38 };
  return profile;
}
function qaRetention(): RetentionProfile {
  ...
  retention.codex = {
    ...
    enemyKills: { goblin: 14, skeleton: 9, orc: 4 },
    bosses: ['warden', 'ash-king'], hunts: ['night-claw'], relics: ['guardian-crown', 'world-core'],
  };
  return retention;
}
function qaPublicProfile(): SocialProfileCardData {
  const equippedItems = normalizeProfileEquipmentItems([
    { slot: 'bow', id: 'ember-bow', level: 3, rarity: 'rare' }, { slot: 'quiver', id: 'black-quiver', level: 2, rarity: 'rare' },
    { slot: 'talisman', id: 'veil-key', level: 5, rarity: 'epic' }, { slot: 'armor', id: 'ash-armor', level: 4, rarity: 'rare' },
  ]);
  return {
    id: 'profile-layout-qa', display_name: 'Schleierjäger', avatar_key: null, friend_code: 'QA-VEIL-27', current_chapter: 7, current_rank: 12,
    character_key: 'archer', last_active_at: new Date().toISOString(), guild_name: 'Wächter des Schleiers', guild_tag: 'VEIL', joined_at: '2026-01-15T12:00:00.000Z',
    account_level: 12, lifetime_world_boss_damage: 248650, world_boss_events: 9, friend_count: 18,
    achievement_keys: ['first_steps', 'veil_walker', 'boss_hunter', 'guild_bound'], activity_state: 'menu', activity_chapter: 7, activity_room: 38,
    highest_chapter: 7, highest_room: 38, rooms_cleared: 146, enemies_defeated: 904, bosses_defeated: 21, quests_completed: 27,
    play_time_ms: 8_640_000, total_damage: 187450, items_found: 14, equipped_items: equippedItems,
  };
}
