...
export function ProfileBadge({ profile, playerName, rank, language, onOpen }: Props) {
  ...
  return <button
    ...
    className="absolute left-3 top-[max(10px,calc(env(safe-area-inset-top)+4px))] z-30 flex h-[48px] w-[min(46vw,176px)] items-center gap-2 rounded-[14px] border px-2 py-1 text-left shadow-[0_8px_22px_rgba(0,0,0,.34)] backdrop-blur-lg active:scale-[.97]"
  ...
  >
    <ProfileAvatarPortrait avatar={avatar} className="h-8 w-8 shrink-0 rounded-[10px] border border-white/16" />
    <div className="min-w-0 flex-1 leading-none">
      <div className="truncate text-[9px] font-black tracking-[.03em] text-white/92">{playerName}</div>
      <div className="mt-1 truncate text-[5.5px] font-black uppercase tracking-[.09em] text-white/50">
        {language === 'de' ? `Rang ${rank}` : `Rank ${rank}`} · {language === 'de' ? title.nameDe : title.nameEn}
      </div>
    </div>
    <span className="shrink-0 text-[11px] text-white/34">›</span>
  </button>;
}
