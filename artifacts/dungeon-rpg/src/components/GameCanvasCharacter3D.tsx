...
export function GameCanvasCharacter3D({ gameState }: { gameState: GameState }) {
  ...
  useEffect(() => {
    ...
    const slimeVariant = (enemy: GameState['enemies'][number]) => {
      ...
      return { color: COLORS[hash % COLORS.length], size: 0.8 + ((hash >>> 3) % 5) * 0.12, squash: 0.52 + ((hash >>> 7) % 4) * 0.06 };
    };
    ...
    const makeSlime = (enemy: GameState['enemies'][number]) => {
      ...
      for (const x of [-0.13, 0.13]) {
        const eye = new THREE.Mesh(new THREE.SphereGeometry(0.052, 6, 5), eyeMat);
    ...
    const makeItem = (type: string) => {
      const group = new THREE.Group();
      if (type === 'xp_orb') {
        const gem = new THREE.Mesh(new THREE.OctahedronGeometry(0.16, 0), mat(0x69d9ff, 0.2, 0x1d8ccb, 1.3));
        gem.rotation.z = 0.35;
        group.add(gem);
      } else {
        const bottle = new THREE.Mesh(new THREE.SphereGeometry(0.16, 8, 7), mat(0xa52c40, 0.35, 0x5d1322, 0.4));
        group.add(bottle);
      }
      return group;
    };
    ...
    const syncWalls = (state: GameState) => {
      ...
      for (let y = 0; y < state.map.height; y++) {
        for (let x = 0; x < state.map.width; x++) {
          ...
          const rock = new THREE.Mesh(new THREE.DodecahedronGeometry(0.38 + (seed % 5) * 0.015, 0), rockMat);
    ...
    boot().catch(error => console.error('Full ranger 3D renderer failed', error));
