...
function DockIcon({ name }: { name: PlaceIcon }) {
  ...
  return <svg viewBox="0 0 24 24" aria-hidden="true" className="h-[18px] w-[18px]">
...
export function VillageNpcHub({ language, dailyProgress, mailUnread, onQuests, onMailbox, onFriends, onGuild }: Props) {
  ...
  return <section data-testid="veil-village-npc-hub" className="relative z-20 mx-auto mt-1.5 w-full max-w-md shrink-0 px-4">
    <div className="grid grid-cols-4 gap-1 rounded-[16px] border border-violet-200/[.09] bg-[#09070d]/82 p-1 shadow-[0_12px_26px_rgba(0,0,0,.36)] backdrop-blur-xl">
      ...
      >
        ...
        <span className="block truncate text-[6.2px] font-black uppercase tracking-[.045em] text-white/70">{de ? place.labelDe : place.labelEn}</span>
      </button>)}
    </div>
  </section>;
}
