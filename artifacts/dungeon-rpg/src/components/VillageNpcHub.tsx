...
function DockIcon({ name }: { name: PlaceIcon }) {
  ...
  return <svg viewBox="0 0 24 24" aria-hidden="true" className="h-8 w-8">
...
export function VillageNpcHub({ language, dailyProgress, mailUnread, onQuests, onMailbox, onFriends, onGuild }: Props) {
  ...
  return <section data-testid="veil-village-npc-hub" className="relative z-20 mx-auto w-full max-w-md px-4">
    <div className="grid grid-cols-4 gap-2 rounded-[22px] border border-violet-200/10 bg-[#09070d]/82 p-2 shadow-[0_18px_38px_rgba(0,0,0,.4)] backdrop-blur-xl">
      {places.map(place => <button
        key={place.testId}
        data-testid={place.testId}
        type="button"
        onPointerDown={event => { event.preventDefault(); place.action(); }}
        className="group relative grid min-h-[78px] min-w-0 place-items-center rounded-2xl border border-white/[.07] bg-[linear-gradient(180deg,rgba(49,35,64,.62),rgba(14,11,19,.78))] px-1.5 py-2 text-center shadow-inner transition active:scale-[.96] active:bg-violet-900/30"
      >
        <span className="mx-auto grid h-10 w-10 place-items-center rounded-xl border border-amber-100/12 bg-black/30 text-amber-100/78 shadow-[inset_0_1px_8px_rgba(255,255,255,.035),0_6px_16px_rgba(0,0,0,.24)] group-active:text-violet-200"><DockIcon name={place.icon} /></span>
        {place.badge && <span className="absolute right-1.5 top-1.5 min-w-4 rounded-full bg-amber-300 px-1 py-0.5 text-[7px] font-black leading-none text-[#251600] shadow-[0_2px_8px_rgba(0,0,0,.4)]">{place.badge}</span>}
        <span className="mt-1 block truncate text-[7px] font-black uppercase tracking-[.08em] text-white/72">{de ? place.labelDe : place.labelEn}</span>
      </button>)}
    </div>
  </section>;
}
