...
async function loadVillageAssets(THREE: any, GLTFLoader: any, scene: any, keepers: any[], isDisposed: () => boolean) {
  ...
  const placements: Record<string, Array<[number, number, number, number, number, string]>> = {
    ...
    mira: [[-2.55, 0, -1.1, 0.98, 0.18, 'MiraQuestKeeper']],
    orin: [[2.55, 0, -1.1, 0.98, -0.18, 'OrinPostKeeper']],
    tala: [[-2.7, 0, -3.55, 0.98, 0.1, 'TalaScoutKeeper']],
    brom: [[2.7, 0, -3.55, 0.98, -0.1, 'BromGuildKeeper']],
    aelric: [[0, 0.08, -4.25, 1.04, 0, 'AelricWorldKeeper']],
  };
  ...
  const results = await Promise.allSettled(VILLAGE_ASSETS.map(async (rule, index) => {
    ...
    if (['mira', 'orin', 'tala', 'brom', 'aelric'].includes(rule.key)) {
...
export function ModernVillageSquareScene() {
  ...
  useEffect(() => {
    ...
    const boot = async () => {
      ...
      camera.position.set(0, 4.45, 10.7);
      camera.lookAt(0, 1.15, -3.75);
      ...
      const resize = () => {
        ...
        camera.fov = camera.aspect < 0.72 ? 38 : 35;
