    ...
    <section className="flex h-[min(94dvh,850px)] w-full max-w-2xl flex-col overflow-hidden rounded-[28px] border border-violet-200/25 bg-[radial-gradient(circle_at_top,rgba(125,72,190,.25),transparent_35%),linear-gradient(160deg,#171020,#09070d)] p-3 text-white" data-testid="guild-raid-run-panel">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto pt-3">
        {!snapshot ? <div className="grid min-h-48 place-items-center text-sm text-white/40">{de ? 'Serverstand wird geladen…' : 'Loading server state…'}</div> : <div className="space-y-3">
          ...
          {bossActive ? <div className="space-y-3" data-testid="guild-raid-boss-panel">
            <div className="rounded-2xl border border-red-200/20 bg-red-500/8 p-3"><div className="flex justify-between text-[8px] uppercase text-white/50"><span>{boss.phase}</span><span>{Math.ceil(boss.health).toLocaleString('de-DE')} / {boss.maxHealth.toLocaleString('de-DE')}</span></div><div className="mt-3 h-3 overflow-hidden rounded-full bg-black/35"><div className="h-full bg-red-300/75" style={{ width: `${bossProgress}%` }} /></div></div>
            ...
            {boss.phase === 'split-echoes' && <div className="grid grid-cols-4 gap-2">{slots.map(echo=><button key={echo} disabled={busy} onClick={()=>void act({type:'bind-echo',slot:mySlot,echo})} className="min-h-16 rounded-xl border border-cyan-200/25 bg-cyan-500/10 text-lg font-black">{echo}</button>)}</div>}
          ...
          </div> : <><div className="rounded-2xl border border-white/10 bg-black/25 p-3"><div className="flex justify-between text-[8px] uppercase text-white/40"><span>Revision {mechanic.revision}</span><span>{mechanic.phase}</span></div><div className="mt-3 h-2 overflow-hidden rounded-full bg-white/8"><div className="h-full bg-violet-300/70" style={{ width: `${Math.min(100, progress)}%` }} /></div></div>
            {!isAdvanced && basicRoom === 1 && <div className="grid grid-cols-2 gap-2">{slots.map(slot => <button key={slot} disabled={busy || slot !== mySlot} onClick={() => void act({ type:'anchor', slot, active: !basic.anchors?.[String(slot)] })} className="min-h-14 rounded-xl border border-violet-200/20 bg-violet-500/10 text-sm font-black disabled:opacity-35">Anker {slot} · {basic.anchors?.[String(slot)] ? 'AKTIV' : 'OFFEN'}</button>)}</div>}
            ...
            {!isAdvanced && basicRoom === 4 && <div className="grid grid-cols-3 gap-2">{['north','south','east'].map(zone=><button key={zone} disabled={busy} onClick={()=>void act({type:'cleanse',slot:mySlot,zoneId:zone,active:true})} className="min-h-16 rounded-xl border border-emerald-200/20 bg-emerald-500/10 uppercase">{zone}</button>)}</div>}
