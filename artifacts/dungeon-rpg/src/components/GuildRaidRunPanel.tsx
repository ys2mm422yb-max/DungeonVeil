...
export function GuildRaidRunPanel({ raidRunId, language, onClose }: Props) {
  ...
  const act = async (action: GuildRaidRoomAction | GuildRaidAdvancedRoomAction) => {
    if (!snapshot || busy) return;
    setBusy(true); setError('');
    try { setSnapshot(isAdvanced ? await submitGuildRaidAdvancedRoomAction(snapshot, action as GuildRaidAdvancedRoomAction) : await submitGuildRaidRoomAction(snapshot, action as GuildRaidRoomAction)); }
    ...
    <section className="flex h-[min(94dvh,850px)] w-full max-w-2xl flex-col overflow-hidden rounded-[28px] border border-violet-200/25 bg-[radial-gradient(circle_at_top,rgba(125,72,190,.25),transparent_35%),linear-gradient(160deg,#171020,#09070d)] p-3 text-white" data-testid="guild-raid-run-panel">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto pt-3">
        {!snapshot ? <div className="grid min-h-48 place-items-center text-sm text-white/40">{de ? 'Serverstand wird geladen…' : 'Loading server state…'}</div> : <div className="space-y-3">
          ...
          <div className="rounded-2xl border border-white/10 bg-black/25 p-3"><div className="flex justify-between text-[8px] uppercase text-white/40"><span>Revision {mechanic.revision}</span><span>{mechanic.phase}</span></div><div className="mt-3 h-2 overflow-hidden rounded-full bg-white/8"><div className="h-full bg-violet-300/70" style={{ width: `${Math.min(100, progress)}%` }} /></div></div>
