    ...
    <div className="overflow-hidden rounded-2xl border border-violet-300/18 bg-[linear-gradient(120deg,rgba(48,29,79,.72),rgba(9,8,12,.9))] shadow-[0_12px_34px_rgba(0,0,0,.3)] backdrop-blur-xl">
      <button type="button" onPointerDown={event => { event.preventDefault(); setOpen(value => !value); }} className="flex w-full items-center gap-3 px-4 py-3 text-left active:scale-[.985]">
        ...
        <div className="min-w-0 flex-1">
          <div className="text-[7px] font-black uppercase tracking-[.22em] text-violet-100/48">{de ? 'WOCHEN-RISS' : 'WEEKLY RIFT'}</div>
          <div className="mt-1 truncate text-[11px] font-black text-white/82">{de ? rift.nameDe : rift.nameEn}</div>
        </div>
      ...
      {open && <div className="border-t border-white/8 px-4 pb-4 pt-3">
        <div className="rounded-xl border border-white/8 bg-black/25 px-3 py-3">
          <div className="text-[10px] font-black" style={{ color: rift.accent }}>{de ? rift.nameDe : rift.nameEn}</div>
          <div className="mt-1.5 text-[8px] font-bold leading-relaxed text-white/48">{de ? rift.ruleDe : rift.ruleEn}</div>
        </div>
        <div className="mt-2 text-[7px] font-black uppercase tracking-[.16em] text-white/30">{de ? `WECHSEL IN ${daysLeft} TAG${daysLeft === 1 ? '' : 'EN'} · SYSTEM VORBEREITET` : `ROTATES IN ${daysLeft} DAY${daysLeft === 1 ? '' : 'S'} · SYSTEM READY`}</div>
      </div>}
    </div>
  );
}
