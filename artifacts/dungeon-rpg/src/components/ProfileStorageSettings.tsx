...
import { loadProfileStorageHealth, type ProfileStorageHealth } from '../game/profileStorageIntegrity';
...
export function ProfileStorageSettings({ language }: { language: 'de' | 'en' }) {
  ...
  return <div data-testid="profile-storage-settings" className="rounded-2xl border border-white/8 bg-white/[.025] p-3">
    <div className="flex items-start justify-between gap-3">
      <div><div className="text-[11px] font-black text-white/86">{de ? 'Profilstatistik' : 'Profile statistics'}</div><div className="mt-1 text-[8px] leading-relaxed text-white/45">{statusLabel(health.status, de)}</div></div>
