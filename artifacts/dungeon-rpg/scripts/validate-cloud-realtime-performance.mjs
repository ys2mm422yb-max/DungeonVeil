...
const checks = [
  ...
  [interpolation.includes('SPECTATOR_BUFFER_CAPACITY = 8') && interpolation.includes('SPECTATOR_INTERPOLATION_DELAY_MS = 140') && interpolation.includes('SPECTATOR_MAX_EXTRAPOLATION_MS = 80') && interpolation.includes('SPECTATOR_MAX_EXTRAPOLATION_PX = 28'), 'timestamped spectator buffer limits are missing'],
  ...
  [spectatorQa.includes('gapPhase >= 16 && gapPhase <= 19') && spectatorQa.includes('SpectatorInterpolationBuffer') && spectatorQa.includes('document.querySelectorAll') && spectatorQa.includes('menuRendererSuspended'), 'deterministic spectator packet-loss QA is missing'],
  ...
  [spectatorTest.includes('toBeLessThanOrEqual(28.1)') && spectatorTest.includes('toBeLessThanOrEqual(8)') && spectatorTest.includes('toBe(1)') && spectatorTest.includes('isContextLost'), 'spectator regression does not enforce bounded extrapolation, React paints, one renderer and healthy WebGL'],
...
console.log('Cloud sync and spectator performance validated: ten-hertz bounded payloads feed a timestamped eight-snapshot buffer, React paints stay throttled, extrapolation is capped and renderer handoff remains exclusive.');
