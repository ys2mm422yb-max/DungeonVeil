...
const explicitEmptySlot = loadout.includes("de ? 'Leer' : 'Empty'")
  || (loadout.includes("de ? 'Nicht ausgerüstet' : 'Not equipped'") && loadout.includes("de ? 'Slot ist leer' : 'Slot is empty'"));
const checks = [
  ...
  [loadout.includes('ACTIVE_EQUIPMENT_SLOTS.map') && loadout.includes('data-equipped') && explicitEmptySlot, 'public equipment cards are not generated from the canonical active slot list with explicit empty slots'],
