...
const [migration, rejoinHardening, online, bridge, session, page, realtime] = await Promise.all([
  read('../../../supabase/migrations/20260719024500_add_coop_run_persistence_and_rewards.sql'),
  read('../../../supabase/migrations/20260719030000_harden_coop_checkpoint_rejoin.sql'),
...
const checks = [
  ...
  [rejoinHardening.includes('authoritative_room_clear boolean') && rejoinHardening.includes('get_my_coop_room_reward_state'), 'cleared-room rejoin or prior claim recovery is missing'],
  [online.includes('COOP_CHECKPOINT_MS = 5_000') && online.includes('createCoopCheckpoint') && online.includes('authoritative_room_clear'), 'five-second Duo checkpoint client or clear-state rejoin lookup is missing'],
  ...
  [bridge.includes('recoverClearedRoom') && bridge.includes('settlePreviouslyClaimedClear') && bridge.includes('getMyCoopRoomRewardState'), 'cleared room, acknowledged reward or boss loot recovery is incomplete'],
...
console.log('Exact-head Duo checkpoints, cleared-room rejoin, per-account rewards, duplicate guards, isolated retries, abort closure and Solo-save isolation validated.');
