...
const checks = [
  ...
  [markers.includes('initializeSeenUnlocks') && inventory.includes('initializeSeenUnlocks') && inventory.includes('inventory-tab-new-badge') && inventory.includes('inventory-item-new-badge') && inventory.includes('dungeon-veil-cloud-save-restored'), 'Block 10 persistent NEW markers are missing from the canonical V4 inventory'],
  ...
  [accessibility.includes('saveAccessibilitySettings') && accessibility.includes("type TextSizeMode = 'standard'") && accessibility.includes("document.documentElement.dataset.textSize = 'standard'") && main.includes('installAccessibilitySettings();') && readability.includes("html[data-contrast='high']") && !readability.includes("html[data-text-size='large']") && !settings.includes('text-size-large'), 'Block 14 contrast or fixed standard-size readability is not integrated'],
