...
const checks = [
  ...
  [inventory.includes('data-testid="equipment-permanent-progression-copy"') && inventory.includes('Ausrüstungslevel 1–5 sind dauerhaft') && inventory.includes('data-testid="equipment-upgrade-preview"') && inventory.includes('data-testid="equipment-upgrade-costs"') && inventory.includes("cost.gold") && inventory.includes("cost.copies") && inventory.includes("cost.dust"), 'equipment levels or their permanent three-resource upgrade path remain unclear'],
  ...
  [inventory.includes('data-testid="relic-source-summary"') && inventory.includes('Bossräumen 10, 20, 30, 40 und 50') && inventory.includes('RELIC_SOURCE_LABELS') && relics.includes("source: 'worldboss'") && !inventory.includes('Bossräume ab Raum 20'), 'relic source explanation does not match all five boss rooms and the world-boss source'],
