...
const checks = [
  ...
  [menuCss.includes("[data-testid='main-menu-dust-button']::after") && menuCss.includes('inset: -14px -2px -2px') && menuCss.includes("[data-testid='main-menu-gold-button']::after") && menuCss.includes('inset: -2px -2px -14px') && menuCss.includes('touch-action: manipulation'), 'compact resource controls do not expose effective 44 px touch hit areas'],
  ...
  [focusedSpec.includes('no-companion-standard-loadout') && focusedSpec.includes('active-companion-ember-raven') && focusedSpec.includes('animation-frame-a') && focusedSpec.includes('animation-frame-b'), 'focused suite lacks companion and multi-frame visual evidence'],
