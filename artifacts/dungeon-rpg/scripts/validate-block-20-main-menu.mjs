...
const checks = [
  ...
  [villagePlayer.includes("showcasePose = 'v15-idle-a-stable-readable-loadout'") && villagePlayer.indexOf("clipKey(clip).includes('idle_a')") >= 0 && villagePlayer.indexOf("!key.includes('idle_b')") > villagePlayer.indexOf("clipKey(clip).includes('idle_a')"), 'village Ranger does not fail closed on the stable Idle_A showcase clip'],
  ...
  [focusedSpec.includes('expectStableRangerIdle') && focusedSpec.includes("animationDriver).toBe('Idle_A')") && focusedSpec.match(/expectStableRangerIdle\(/g)?.length === 4, 'focused suite does not enforce the stable Ranger animation for every loadout state'],
