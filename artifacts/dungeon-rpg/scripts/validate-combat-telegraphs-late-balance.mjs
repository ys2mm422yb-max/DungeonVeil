...
assert(balance.includes('Math.min(CHAPTER_ROOMS, engine.state.floor)'), 'Late-room balance must use the full authored chapter length.');
assert(!balance.includes('Math.min(20, engine.state.floor)'), 'Room 21-60 must not be collapsed into room 20.');
assert(balance.includes('1 + Math.max(0, room - 1) * 0.022'), 'Central HP pressure must rise across all sixty rooms.');
assert(balance.includes('1 + Math.max(0, room - 1) * 0.0105'), 'Central attack pressure must rise across all sixty rooms.');
...
assert(balance.includes('hpFloor: 2200') && balance.includes('hpFloor: 3200') && balance.includes('hpFloor: 4500') && balance.includes('hpFloor: 6200'), 'Rooms 30, 40, 50 and 60 need distinct boss HP floors.');
assert(balance.includes('attackCap: 48') && balance.includes('attackCap: 60') && balance.includes('attackCap: 72') && balance.includes('attackCap: 82'), 'Rooms 30, 40, 50 and 60 need distinct boss attack caps.');

const hpFactor = room => 1 + Math.max(0, room - 1) * 0.022;
for (let room = 2; room <= 60; room++) {
...
console.log('Combat telegraph and centralized room 21-60 balance audit passed.');
