...
const runtimeUpdateStart = equipmentRuntime.indexOf('export function updateEquipmentRuntimeBalance');
assert(runtimeUpdateStart >= 0, 'equipment runtime update function is missing');
const runtimeUpdateSource = equipmentRuntime.slice(runtimeUpdateStart);
const chapterRuntimeCall = runtimeUpdateSource.indexOf('applyChapterEncounterRuntimeV4');
const combatOverlayCall = runtimeUpdateSource.indexOf('applyCombatBalanceV4Overlay');
assert(chapterRuntimeCall >= 0 && combatOverlayCall >= 0 && chapterRuntimeCall < combatOverlayCall, 'chapter roles are applied after combat scaling');
