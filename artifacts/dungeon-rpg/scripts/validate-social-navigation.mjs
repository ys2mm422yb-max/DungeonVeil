...
const checks = [
  ...
  [!menuSceneProxy.includes('VeilWorldOrb') && !villageScene.includes('VeilWorldGlobe') && !villageScene.includes('buildVeilWorldOrb'), 'legacy world-globe menu markers returned'],
  ...
  [villageScene.includes("window.visualViewport?.addEventListener('resize', resize)") && villageScene.includes('camera.position.set(0, 6.25, 14.6)'), 'mobile village viewport or camera composition regressed'],
  [villageScene.includes("aelric: [[0, 0.02, -5.65, 0.34, 0, 'AelricWorldKeeper']]"), 'Aelric is no longer scaled and placed behind the shrine'],
  [villageHub.includes('grid grid-cols-5') && villageHub.includes('Choose a place') && !villageHub.includes('absolute z-20 flex') && !villageHub.includes('top="47.5%"'), 'village place dock is missing or floating labels returned'],
  [menu.includes('grid-cols-2') && menu.includes('min-h-[250px] flex-1') && !menu.includes('h-[41vh]'), 'main-menu action layout regressed to the old fixed-height composition'],
...
console.log('Social/navigation audit passed: the modern mobile village square is active and legacy world-globe, fixed-height and floating-label regressions are blocked.');
