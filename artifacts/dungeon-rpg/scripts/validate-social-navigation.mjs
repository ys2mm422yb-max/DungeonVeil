...
const checks = [
  ...
  [villageScene.includes('raf = requestAnimationFrame(loop)') && villageScene.includes('void loadVillageAssets(') && villageScene.indexOf('raf = requestAnimationFrame(loop)') < villageScene.indexOf('void loadVillageAssets('), 'village renderer does not start before asynchronous asset loading'],
