...
const checks = [
  ...
  [!menu.includes('<GuildInviteLinkCard') && inviteCard.includes('createGuildInviteLinkOnline') && inviteCard.includes('navigator.share') && menu.includes('onClose={() => setOverlay(null)}'), 'guild link sharing is not isolated inside the closable guild panel'],
  ...
  [villageScene.includes("window.visualViewport?.addEventListener('resize', resize)") && villageScene.includes('camera.position.set(0, 6.25, 14.6)') && villageScene.includes("'AelricWorldKeeper'"), 'mobile village viewport resize handling is missing'],
