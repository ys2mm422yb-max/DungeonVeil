...
requireText(spec, /const CHAPTER_ROOMS = 100/, 'The evidence suite must bind chapter evidence to 100 rooms.');
requireText(spec, /const CHUNKS = Array\.from\(\{ length: 10 \}/, 'The evidence suite must split all 100 rooms into ten reviewable chunks.');
...
requireText(spec, /RAUM\\s\*\$\{room\}\/\$\{CHAPTER_ROOMS\}/, 'The evidence suite must verify the 100-room HUD contract through the shared chapter-room constant.');
...
requireText(postClearSpec, /killAndTrackPostClearHp/, 'The post-clear regression must capture baseline and remove the final living enemies atomically.');
requireText(postClearSpec, /result\.probe\.minimum[\s\S]*toBeGreaterThanOrEqual\(result\.probe\.baseline\)/, 'The post-clear regression must reject every frame-level HP drop after the final kill.');
...
requireText(atomicReadySpec, /blocked\.hpProbe\.minimum[\s\S]*toBeGreaterThanOrEqual\(blocked\.hpProbe\.baseline\)/, 'A failed build must keep hazards and HP frozen after guard activation.');
