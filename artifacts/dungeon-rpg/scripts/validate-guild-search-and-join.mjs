...
const checks = [
  ...
  [migration.includes('exists (select 1 from public.guild_members where user_id = v_user_id)') && migration.includes('guild_members_user_id_key'), 'guild join does not enforce one membership per player'],
