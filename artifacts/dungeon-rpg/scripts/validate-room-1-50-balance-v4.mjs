...
function encounter(room) {
  ...
  if (region === 5 && !plan.some(type => type === 'orc' || type === 'golem') && plan.length > 0) plan[0] = 'golem';
  return plan;
}
...
for (let room = 1; room <= 50; room++) {
  ...
  if (room >= 21) assert(new Set(plan.map(type => ENEMY[type].role)).size >= 3, `room ${room} lacks mixed role pressure`);
...
const bandRoles = (from, to) => new Set(rows
  .filter(row => row.chapter === 5 && row.room >= from && row.room <= to && !row.boss)
  .flatMap(row => row.roles));
assert(bandRoles(21, 29).size === 4 && bandRoles(31, 39).size === 4 && bandRoles(41, 49).size === 4, 'mid or late room band does not cover all combat roles');
assert(bandAverage(41, 49, 'pressure') > bandAverage(1, 9, 'pressure') * 2.4, 'late-room pressure does not meaningfully exceed the opening band');
...
assert(encounterSource.includes('ensureLateRoomRolePressure') && encounterSource.includes("plan[0] = 'golem'"), 'late-room front-line correction is not canonical');
