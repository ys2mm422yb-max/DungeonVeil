...
const [panel, card, client, identity, progress] = await Promise.all([
...
const checks = [
  ...
  [card.includes('public-player-profile-tablet-columns') && card.includes('public-player-profile-equipment') && card.includes('public-equipment-detail'), 'friend profiles do not use the responsive tablet layout and current equipment detail'],
  [progress.includes('normalizePublicEquipmentItems') && !card.toLowerCase().includes('talisman'), 'friend profiles can still expose retired equipment slots'],
...
console.log('Friends/public-profile audit passed: cosmetic friend and request cards open responsive current-loadout profiles, legacy slots stay hidden, actions remain separate and active runs can be watched.');
