...
assert(worldLoot.includes("'dungeon-veil-duo-loot-proposal'") && worldLoot.includes('BEUTEENTSCHEIDUNG AUSSTEHEND'), 'Boss equipment is not routed into the shared decision or portal guard.');
...
assert(client.includes("state.winner_user_id === userId") && client.includes('collectBalancedEquipmentDrop(state.item_id)'), 'The resolved winner does not receive exactly one local item.');
...
assert(client.includes('rewardLedger.includes(ledgerKey)') && client.includes("coop-loot:${state.lobby_id}"), 'Reconnects can apply a resolved reward more than once.');
...
assert(overlay.includes('POLL_MS = 700') && overlay.includes('loadCoopSharedLoot'), 'Reconnect-safe shared loot polling is missing.');
...
assert(migration.includes("v_loot.resolve_after > v_now") && migration.includes("coalesce(v_loot.choices ->> member_id::text, 'pass')"), 'Missing or disconnected players do not safely time out as pass.');
...
console.log('Duo equipment uses one host-authored drop, individual currency, claim/pass, server-side tie rolls, loser compensation, reconnect-safe polling and one-winner delivery.');
