...
const checks = [
  ...
  [migration.includes("set status = 'abandoned'") && migration.includes('event_type') && migration.includes("'run_abandoned'"), 'authoritative abort event or terminal abandoned-state transition is missing'],
  ...
  [client.includes('nextVersion > knownVersion + 1') && client.includes('postgres_changes') && client.includes('heartbeat'), 'Realtime version-gap reconciliation or heartbeat is missing'],
