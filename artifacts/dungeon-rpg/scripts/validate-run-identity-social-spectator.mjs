...
const checks = [
  ...
  [spectatorScreen.includes('SpectatorInterpolationBuffer') && spectatorScreen.includes('SPECTATOR_UI_PAINT_MS') && spectatorScreen.includes('spectator-health') && spectatorScreen.includes('spectator-gifts') && spectatorScreen.includes('heartbeatSpectatorViewer'), 'spectator view lacks buffered smoothing, throttled UI, health, gifts or viewer presence'],
  [spectatorInterpolation.includes('SPECTATOR_INTERPOLATION_DELAY_MS = 140') && spectatorInterpolation.includes('SPECTATOR_MAX_EXTRAPOLATION_MS = 80') && spectatorInterpolation.includes('SPECTATOR_BUFFER_CAPACITY = 8'), 'spectator interpolation delay, extrapolation cap or buffer bound is missing'],
...
console.log('Run identity/social spectator audit passed: account names, bounded 10Hz viewing, timestamped smoothing, exclusive mobile WebGL handoff, viewer counts, gifts, lifecycle status and public equipment are integrated.');
