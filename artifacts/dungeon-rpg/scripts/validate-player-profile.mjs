...
const checks = [
  ...
  [files.panel.includes('data-testid="player-profile-tabs"') && files.panel.includes('grid-cols-5') && files.panel.includes('min-w-0') && files.panel.includes('aria-label={item.full}'), 'player profile tabs are not five stable, readable full-width controls'],
  [files.panel.includes('testId="profile-distinct-equipment-count"') && files.panel.includes('activeOwnedEquipmentCount(meta)') && files.panel.includes('data-testid="profile-collection-summary"'), 'distinct active equipment ownership is unclear or includes retired equipment'],
  [files.panel.includes('testId="profile-lifetime-equipment-rewards"') && files.panel.includes('Ausrüstungsbelohnungen insgesamt'), 'lifetime equipment rewards are not distinguished from unique owned items'],
  ...
  [files.loadout.includes('ACTIVE_EQUIPMENT_SLOTS.map') && files.loadout.includes('<KayKitEquipmentPreview') && files.loadout.includes('<EquipmentArtwork') && (files.loadout.match(/<KayKitEquipmentPreview\b/g) ?? []).length === 1, 'profile equipment loadout does not use three canonical slots, shared equipment artwork and one controlled item renderer'],
