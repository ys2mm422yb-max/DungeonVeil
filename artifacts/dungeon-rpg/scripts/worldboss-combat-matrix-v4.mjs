...
function mitigation(defense) {
  return Math.min(BOSS.armorCap, Math.max(0, defense) / (Math.max(0, defense) + 32));
}
function simulate(buildName, relicName, giftName) {
  ...
  const dps = (build.attack + gifts.attackFlat) * relic.attack * expectedCrit * attacksPerSecond * effectiveArrows * gifts.followUp * rangeUptime;
  const clearSeconds = BOSS.health / Math.max(1, dps);
  const damage = Math.min(BOSS.health, dps * BOSS.timeLimit);
  const damagePercent = damage / BOSS.health;
  const weightedRawAttack = BOSS.fireBreath * 0.42 + BOSS.claw * 0.38 + BOSS.slam * 0.20;
  const incomingDps = weightedRawAttack * (1 - mitigation(build.defense)) * (1 - relic.reduction * 0.4) * BOSS_HIT_UPTIME;
  const fightSeconds = Math.min(BOSS.timeLimit, clearSeconds);
  const damageTaken = incomingDps * fightSeconds;
  const effectiveHp = (build.hp + gifts.hpFlat) * relic.hp;
  return {
    build: buildName, relic: relicName, gifts: giftName,
    dps, clearSeconds, damage, damagePercent, cooldown,
    mitigation: mitigation(build.defense), damageTaken, effectiveHp,
    survives: damageTaken < effectiveHp,
...
export function simulateWorldBossCombatMatrixV4() {
  ...
  for (const build of Object.keys(BUILDS)) {
    for (const relic of Object.keys(RELICS)) {
      for (const gifts of Object.keys(GIFTS)) rows.push(simulate(build, relic, gifts));
    }
  }
