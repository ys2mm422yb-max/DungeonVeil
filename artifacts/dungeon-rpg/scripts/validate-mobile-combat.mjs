...
const files = {
  ...
  loading: await readFile(new URL('../src/components/LoadingScreen.tsx', import.meta.url), 'utf8'),
  globalLoading: await readFile(new URL('../src/components/GlobalLoadingLayer.tsx', import.meta.url), 'utf8'),
  ...
  boss: await readFile(new URL('../src/components/WorldBossBattleScreenV4.tsx', import.meta.url), 'utf8'),
  ...
  bossArenaGuard: await readFile(new URL('../src/components/WorldBossMobileArenaGuard.tsx', import.meta.url), 'utf8'),
...
const checks = [
  ...
  [files.loading.includes('data-boot-presentation="veil-gate"') && files.loading.includes('app-boot-progress') && files.loading.includes('SCHLEIERSTATUS') && files.globalLoading.includes('APP_BOOT_READY_EVENT') && files.globalLoading.includes('setBootProgress(100)'), 'the app still uses a generic boot card instead of the staged Veil gate presentation'],
  [files.accessibility.includes("type ContrastMode = 'standard' | 'high'") && files.accessibility.includes("type TextSizeMode = 'standard'") && files.accessibility.includes('saveAccessibilitySettings'), 'contrast persistence or the fixed standard UI size is missing'],
  [files.accessibility.includes("DEFAULT_SETTINGS: AccessibilitySettings = { contrast: 'high', textSize: 'standard' }") && files.accessibility.includes('SETTINGS_VERSION = 3') && files.accessibility.includes("textSize: 'standard'") && files.accessibility.includes('markSettingsActivity'), 'accessibility defaults or standard-size migration are incorrect'],
  [files.accessibility.includes('document.documentElement.dataset.contrast') && files.accessibility.includes("document.documentElement.dataset.textSize = 'standard'"), 'accessibility settings are not applied with a fixed standard UI size'],
  [files.settings.includes('accessibility-settings') && files.settings.includes('contrast-mode-high') && !files.settings.includes('text-size-large') && !files.settings.includes('UI-Schrift'), 'the removed large UI option is still present in settings'],
  ...
  [files.readability.includes('button:focus-visible') && files.readability.includes("html[data-contrast='high']") && !files.readability.includes("html[data-text-size='large']"), 'focus or high-contrast rules are incomplete, or obsolete large UI rules remain'],
  ...
  [files.bossStage.includes('camera.fov = phonePortrait ? 58') && files.bossStage.includes('cameraDistance = phonePortrait ? 23.6') && files.bossStage.includes('playerX * (phonePortrait ? 0.045 : 0.12)') && files.bossStage.includes('data-phone-framing="adaptive"'), 'world-boss phone framing is still zoomed in or follows the player too aggressively'],
