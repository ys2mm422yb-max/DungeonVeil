...
const [policy, identity, main] = await Promise.all([
  read('../src/game/darkwoodMageProjectilePolicy.ts'),
  read('../src/game/enemyRegionalIdentity.ts'),
  read('../src/main.tsx'),
]);

const checks = [
  [main.includes("import './game/darkwoodMageProjectilePolicy';"), 'darkwood mage projectile policy is not installed'],
  ...
  [identity.includes("if (type === 'vampire') return skeleton('mage', 'mage');"), 'darkwood vampire caster does not use the dedicated mage model'],
...
console.log('Darkwood mage projectile audit passed: dedicated mage identity, visible flight time, delayed impact and dodgeable damage are installed.');
