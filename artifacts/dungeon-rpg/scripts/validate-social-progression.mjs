...
const [migration, noticesMigration, rewardSweepMigration, profileExtensionMigration, attemptMigration, socialClient, friendClient, attemptClient, friendsPanel, guildSocial, profileCard, onlinePanel, bossPanel, mailbox, rewardLocal, tutorial, tutorialState, bridge, menu, villageHub, menuScene, villageScene, main] = await Promise.all([
...
const checks = [
  ...
  [guildSocial.includes('GuildPanelMobile') && guildSocial.includes('guild-profile-list-button') && guildSocial.includes('PlayerProfileCard') && guildSocial.includes('onClose') && menu.includes('<GuildSocialPanel'), 'guild member profiles or fixed close routing are missing'],
  ...
  [menu.includes('<VillageNpcHub') && menuScene.includes('ModernVillageSquareScene') && villageScene.includes("villageRoot.name = 'ModernKayKitVillageSquare'") && villageScene.includes("rig.root.name = 'VillageEquippedPlayer'"), 'village NPC navigation is not backed by the modern equipped-player scene'],
...
console.log('Social progression audit passed: the server-enforced boss gate, equipped village player and fixed mobile guild shell are wired with the existing social progression.');
