...
const checks = [
  ...
  [menu.includes('<VillageNpcHub') && menuScene.includes('ModernVillageSquareScene') && menuScene.includes('SPECTATOR_RENDERER_EVENT') && villageSceneProxy.includes('HallOfVeilHybridBackground') && villageSceneProxy.includes('HallOfVeilScene') && hallScene.includes("hallRoot.userData.sceneContract = 'hall-of-the-veil-v5-hybrid'") && hallScene.includes("backgroundMode = 'premium-2d-artwork'") && hallScene.includes('loadKayKitVillageArcher') && hallScene.includes('loadKayKitMenuCompanion') && hallScene.includes('marketStalls: 0') && hallScene.includes('decorativeNpcs: 0') && villagePlayer.includes("root.name = 'VillageEquippedPlayer'") && villagePlayer.includes('KAYKIT_PLAYER_ASSETS.ranger') && villagePlayer.includes('village-showcase-v14-player-focus') && villagePlayer.includes("equipmentRoot.name = 'VillageReadableLoadout'") && villagePlayer.includes('root.scale.setScalar(0.72)'), 'social navigation is not backed by the premium hybrid Hall with the equipped Ranger and active companion'],
...
console.log('Social progression audit passed: public career profiles, friend-only live viewing, server boss gates and the premium hybrid Hall remain integrated.');
