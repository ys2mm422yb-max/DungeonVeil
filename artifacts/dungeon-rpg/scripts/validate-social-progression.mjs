...
const checks = [
  ...
  [guildSocial.includes('GuildPanelMobile') && guildSocial.includes('PlayerProfileCard') && guildMemberProfileRouting && guildPanel.includes('data-testid="guild-members-tab"') && guildMemberProfileButton && guildPanel.includes('SocialIdentityCard') && !guildSocial.includes('guild-profile-list-button') && !guildSocial.includes('guild-member-profile-strip') && guildSocial.includes('onClose') && menu.includes('<GuildSocialPanel'), 'guild cosmetic member profiles are not contained in the Members tab or fixed close routing is missing'],
