...
const [main, packageJson, roomSuite, progression, navigation, portal, villageProxy, hall, hallBackground, menuCompanion, villagePlayer, guild, roomAudit, roomComposition, bossClient] = await Promise.all([
  read('../src/main.tsx'),
  read('../package.json'),
...
const checks = [
  ...
  [villageProxy.includes('HallOfVeilHybridBackground') && villageProxy.includes('HallOfVeilScene') && !villageProxy.includes('mix-blend-screen') && hall.includes('loadKayKitVillageArcher') && hall.includes('loadKayKitMenuCompanion') && hall.includes("hallRoot.userData.sceneContract = 'hall-of-the-veil-v6-premium'") && hall.includes("hallRoot.userData.backgroundMode = 'premium-2d-artwork'") && hall.includes('playerFullBody: true') && hall.includes('companionFullBody: true') && hall.includes('marketStalls: 0') && hall.includes('decorativeNpcs: 0') && hallBackground.includes('premium-gothic-v3') && menuCompanion.includes('HallActiveCompanion_') && villagePlayer.includes("root.name = 'VillageEquippedPlayer'") && villagePlayer.includes('KAYKIT_PLAYER_ASSETS.ranger') && villagePlayer.includes('village-showcase-v14-player-focus') && villagePlayer.includes("equipmentRoot.name = 'VillageReadableLoadout'") && villagePlayer.includes('root.scale.setScalar(0.72)'), 'premium Hall is missing crisp artwork, the focused equipped Ranger or the active companion'],
...
console.log('Combined integration audit passed: legacy Blocks 1–8 and current Blocks 4–17 remain compatible with the premium Hall in one build.');
