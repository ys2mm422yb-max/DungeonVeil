...
const protectedSoloFiles = new Map([
  ...
  ['../src/game/equipmentTargeting.ts', '87d976e570c9480dfb18a966906dd8251c324dd8'],
...
const [runMode, targeting] = await Promise.all([
...
const contractChecks = [
  ...
  [targeting.includes('export function grantEquipmentSourceMarkOnce(')
    && targeting.includes('equipment-source-mark:')
    && targeting.indexOf('export function grantEquipmentSourceMarkOnce(') > targeting.indexOf('export function grantEquipmentSourceMark('), 'the approved append-only Duo source-mark helper is missing or replaced Solo targeting logic'],
];
...
console.log(`Co-op isolation passed: seven Solo balance files remain byte-identical, equipment targeting has only the locked append-only Duo mark helper, and Duo uses a separate run context.`);
