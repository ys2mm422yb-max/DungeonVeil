...
assert(preview.includes('let sharedRenderer: any = null;') && preview.includes('getSharedRenderer(THREE, enemyType)') && preview.includes("window.addEventListener('pagehide', () => releaseSharedRenderer('pagehide'), { once: true })"), 'Codex selections recreate WebGL contexts instead of reusing one page-bounded renderer.');
