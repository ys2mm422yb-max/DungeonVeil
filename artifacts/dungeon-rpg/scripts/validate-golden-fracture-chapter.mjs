...
assert(
  encounters.includes('Math.min(CHAPTER_ROOMS, room)')
    || encounters.includes('Math.min(CHAPTER_ROOMS, Math.floor(room))'),
  'encounter lookup is still clamped below chapter length',
);
