...
assert.match(journey, /const captureBoundaryState = await captureBoundaryHandle\.jsonValue\(\);[\s\S]*expect\(captureBoundaryState\.playerLastAttackTime\)\.toBe\(captureBoundaryState\.observedPlayerAttackAt\);[\s\S]*const replenishedCriticalRoom = await page\.evaluate\(\(\) => window\.__dungeonVeilRuntimeEvidence\?\.loadRoom\(1, 'solo'\) \?\? null\);[\s\S]*expect\(Number\(replenishedCriticalRoom\?\.livingEnemies \|\| 0\)\)\.toBeGreaterThan\(0\);[\s\S]*await prepareLivePlayerAttackLine\(page\);[\s\S]*const confirmedPlayerAttackAt = await triggerConfirmedPlayerAttack/,
  'critical capture must replenish and re-prove a durable authoritative target after the final readiness boundary and immediately before confirmed input');
...
assert.ok(
  ...
  && criticalCaptureBoundaryAt > criticalReadyBoundaryAt
  && criticalRoomReplenishAt > criticalCaptureBoundaryAt
  && criticalInputAt > criticalRoomReplenishAt,
  'target replenishment must happen after the final strict pre-input boundary and before confirmed input',
);
