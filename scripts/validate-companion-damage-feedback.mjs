...
assert.match(atomicPlayerAttackReader, /page\.evaluate\(\(\{ boundary, setterKey \}\) => \{[\s\S]*window\.__dungeonVeilRuntimeEvidence\?\.snapshot\(\) \?\? null[\s\S]*const confirmedAt = Number\(snapshot\?\.playerLastAttackTime \|\| 0\);[\s\S]*if \(confirmedAt <= boundary\) return \{ snapshot, confirmedAt, armed: false \};[\s\S]*const armed = setterKey \? window\[setterKey\]\?\.\(confirmedAt\) === true : true;[\s\S]*return \{ snapshot, confirmedAt, armed \};/,
  'authoritative player-attack observation and trigger-floor arming must occur in one browser task');
...
assert.match(journey, /expect\(observedCritical\.criticalPlayerAttackAt\)\.toBeGreaterThanOrEqual\(confirmedPlayerAttackAt\);/,
  'the accepted critical feedback must name an authoritative actual proc source at or after the confirmed trigger lower bound');
