...
assert.match(atomicPlayerAttackReader, /page\.evaluate\(\(\{ boundary, setterKey \}\) => \{[\s\S]*window\.__dungeonVeilRuntimeEvidence\?\.snapshot\(\) \?\? null[\s\S]*companion-runtime-bridge[\s\S]*playerLastAttackTime = Number\(snapshot\?\.playerLastAttackTime \|\| 0\)[\s\S]*observedPlayerAttackAt[\s\S]*criticalPlayerAttackAt[\s\S]*playerLastAttackTime > boundary[\s\S]*observedPlayerAttackAt === playerLastAttackTime[\s\S]*criticalPlayerAttackAt === playerLastAttackTime[\s\S]*window\[setterKey\]\?\.\(confirmedAt\) === true/,
  'authoritative critical-source observation may arm only after runtime snapshot, consumed attack marker and actual critical source converge exactly in one browser task');
...
assert.match(playerAttackTrigger, /const inputBurst = 6;[\s\S]*readConfirmedPlayerAttackAndArm\(page, attackBoundary, expectedPlayerAttackSetterKey\)[\s\S]*livingEnemyPositions[\s\S]*moveWithKeyboard\(page, keys, durationMs\)[\s\S]*page\.keyboard\.press\('Space'\)[\s\S]*confirmedAt > attackBoundary[\s\S]*No authoritative player attack occurred/,
  'the bounded input search must finish only after the exact critical-producing player source converges strictly beyond the captured boundary and is atomically armed');
assert.match(playerAttackTrigger, /await moveWithKeyboard\(page, keys, durationMs\);[\s\S]*const movementState = await readConfirmedPlayerAttackAndArm\(page, attackBoundary, expectedPlayerAttackSetterKey\);[\s\S]*const movementAttackAt = Number\(movementState\.confirmedAt \|\| 0\);[\s\S]*if \(movementAttackAt > attackBoundary\) \{[\s\S]*if \(!movementState\.armed\) throw new Error\([\s\S]*return movementAttackAt;[\s\S]*\}[\s\S]*await page\.keyboard\.press\('Space'\);/,
  'the input search must atomically arm and return the first exact critical source produced during target approach before issuing another attack input');
...
assert.match(journey, /const replenishedCriticalRoom = await page\.evaluate\(\(\) => window\.__dungeonVeilRuntimeEvidence\?\.loadRoom\(1, 'solo'\) \?\? null\);[\s\S]*expect\(Number\(replenishedCriticalRoom\?\.livingEnemies \|\| 0\)\)\.toBeGreaterThan\(0\);[\s\S]*await waitForStableRoom\(page\);[\s\S]*await prepareLivePlayerAttackLine\(page\);[\s\S]*const captureBoundaryState = await captureBoundaryHandle\.jsonValue\(\);[\s\S]*expect\(captureBoundaryState\.playerLastAttackTime\)\.toBe\(captureBoundaryState\.observedPlayerAttackAt\);[\s\S]*const expectedPlayerAttackSetterKey = '__dungeonVeilCriticalCompanionFeedbackObservationSetExpectedPlayerAttackAt';[\s\S]*const confirmedPlayerAttackAt = await triggerConfirmedPlayerAttack/,
  'critical capture must replenish and re-prove a durable authoritative target before the final strict capture boundary and atomic confirmed input');
...
assert.match(journey, /expect\(observedCritical\.playerLastAttackTime\)\.toBe\(confirmedPlayerAttackAt\);[\s\S]*expect\(observedCritical\.observedPlayerAttackAt\)\.toBe\(confirmedPlayerAttackAt\);/,
  'the accepted payload must preserve exact convergence of snapshot and consumed-player marker with the critical source');
