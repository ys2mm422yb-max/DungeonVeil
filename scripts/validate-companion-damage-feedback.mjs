...
assert.match(journey, /attributeFilter: \[[\s\S]*'data-last-critical-special-player-attack-at'[\s\S]*'data-last-observed-player-attack-at'[\s\S]*\]/,
  'the atomic observer must re-inspect when either runtime source marker converges');
...
const atomicPlayerAttackReader = journey.match(/async function readConfirmedPlayerAttack\(page, attackBoundary\) \{[\s\S]*?\n\}/)?.[0] ?? '';
assert.match(atomicPlayerAttackReader, /page\.evaluate\(boundary => \{[\s\S]*window\.__dungeonVeilRuntimeEvidence\?\.snapshot\(\) \?\? null[\s\S]*companion-runtime-bridge[\s\S]*playerLastAttackTime = Number\(snapshot\?\.playerLastAttackTime \|\| 0\)[\s\S]*observedPlayerAttackAt[\s\S]*criticalPlayerAttackAt[\s\S]*playerLastAttackTime > boundary[\s\S]*observedPlayerAttackAt === playerLastAttackTime[\s\S]*criticalPlayerAttackAt === playerLastAttackTime[\s\S]*return \{ snapshot, confirmedAt, playerLastAttackTime, observedPlayerAttackAt, criticalPlayerAttackAt \};/,
  'authoritative player-attack confirmation must observe one exact converged snapshot/consumed/critical timestamp without mutating the browser observer state');
assert.doesNotMatch(atomicPlayerAttackReader, /setterKey|armed|SetExpectedPlayerAttackAt/,
  'postcondition observation must not depend on re-arming browser state after the observer may already have captured and cleaned up');

assert.match(journey, /const exactPlayerAttackAt = \([\s\S]*playerLastAttackTime === criticalPlayerAttackAt[\s\S]*observedPlayerAttackAt === criticalPlayerAttackAt[\s\S]*\) \? criticalPlayerAttackAt : 0;[\s\S]*if \(!Number\.isFinite\(expectedPlayerAttackAt\)\) \{[\s\S]*scope\[expectedPlayerAttackStateKey\] = exactPlayerAttackAt;[\s\S]*expectedPlayerAttackAt = exactPlayerAttackAt;[\s\S]*\}[\s\S]*criticalPlayerAttackAt !== expectedPlayerAttackAt[\s\S]*playerLastAttackTime !== expectedPlayerAttackAt[\s\S]*observedPlayerAttackAt !== expectedPlayerAttackAt/,
  'the browser-local observer must atomically derive, lock and fail-closed on one exact critical source before freshness acceptance');

const playerAttackTrigger = journey.match(/async function triggerConfirmedPlayerAttack\(page, attackBoundary\) \{[\s\S]*?\n\}/)?.[0] ?? '';
assert.match(playerAttackTrigger, /const inputBurst = 6;[\s\S]*readConfirmedPlayerAttack\(page, attackBoundary\)[\s\S]*livingEnemyPositions[\s\S]*moveWithKeyboard\(page, keys, durationMs\)[\s\S]*page\.keyboard\.press\('Space'\)[\s\S]*confirmedAt > attackBoundary[\s\S]*No authoritative player attack occurred/,
  'the bounded input search must finish only after an exact runtime-confirmed critical-producing player source converges strictly beyond the captured boundary');
assert.match(playerAttackTrigger, /await moveWithKeyboard\(page, keys, durationMs\);[\s\S]*const movementState = await readConfirmedPlayerAttack\(page, attackBoundary\);[\s\S]*const movementAttackAt = Number\(movementState\.confirmedAt \|\| 0\);[\s\S]*if \(movementAttackAt > attackBoundary\) return movementAttackAt;[\s\S]*await page\.keyboard\.press\('Space'\);/,
  'the input search must return the first exact source observed during target approach before issuing another attack input');
...
assert.doesNotMatch(playerAttackTrigger, /could not be armed atomically|setterKey|expectedPlayerAttackSetterKey|\.armed/,
  'the later Playwright confirmation must remain observation-only and must not restore the lifecycle race');
...
assert.match(journey, /await prepareLivePlayerAttackLine\(page\);\s*const attackBoundary = Number\(\(await readRuntimeCombatSnapshot\(page\)\)\?\.playerLastAttackTime \|\| 0\);\s*const capturePromise = captureLiveCompanionFeedbackEvidence\(page, \{[\s\S]*role: 'critical-support',[\s\S]*critical: true,[\s\S]*notBefore: attackBoundary,[\s\S]*marker: \/✦\\s\*-\\d\+\/[\s\S]*companion-damage-feedback-critical-\$\{testInfo\.project\.name\}\.png[\s\S]*__dungeonVeilCriticalCompanionFeedbackObservationArmed[\s\S]*const atomicReadyBoundaryHandle = await page\.waitForFunction\(\(\{ setter \}\) => \{[\s\S]*data-critical-special-ready[\s\S]*const evidenceBoundary = performance\.now\(\);[\s\S]*window\[setter\]\?\.\(evidenceBoundary\) !== true[\s\S]*playerLastAttackTime[\s\S]*return \{ evidenceBoundary, playerLastAttackTime \};[\s\S]*__dungeonVeilCriticalCompanionFeedbackObservationSetMinimumAt[\s\S]*timeout: 20_000,[\s\S]*polling: 16[\s\S]*const atomicReadyBoundary = await atomicReadyBoundaryHandle\.jsonValue\(\);[\s\S]*const readyAttackBoundary = Number\(atomicReadyBoundary\.playerLastAttackTime \|\| 0\);[\s\S]*const replenishedCriticalRoom = await page\.evaluate\(\(\) => window\.__dungeonVeilRuntimeEvidence\?\.loadRoom\(1, 'solo'\) \?\? null\);[\s\S]*expect\(Number\(replenishedCriticalRoom\?\.livingEnemies \|\| 0\)\)\.toBeGreaterThan\(0\);[\s\S]*await waitForStableRoom\(page\);[\s\S]*await prepareLivePlayerAttackLine\(page\);[\s\S]*const captureBoundaryHandle = await page\.waitForFunction[\s\S]*data-critical-special-ready[\s\S]*data-last-observed-player-attack-at[\s\S]*playerLastAttackTime !== observedPlayerAttackAt[\s\S]*window\[setter\]\?\.\(captureBoundary\) !== true[\s\S]*const captureBoundaryState = await captureBoundaryHandle\.jsonValue\(\);[\s\S]*expect\(captureBoundaryState\.playerLastAttackTime\)\.toBe\(captureBoundaryState\.observedPlayerAttackAt\);[\s\S]*const confirmedPlayerAttackAt = await triggerConfirmedPlayerAttack\([\s\S]*Math\.max\(readyAttackBoundary, captureBoundary, captureBoundaryState\.playerLastAttackTime\),[\s\S]*\);[\s\S]*const observedCritical = await capturePromise;[\s\S]*expect\(observedCritical\.criticalPlayerAttackAt\)\.toBe\(confirmedPlayerAttackAt\);[\s\S]*expect\(observedCritical\.playerLastAttackTime\)\.toBe\(confirmedPlayerAttackAt\);[\s\S]*expect\(observedCritical\.observedPlayerAttackAt\)\.toBe\(confirmedPlayerAttackAt\);[\s\S]*expect\(observedCritical\.expectedPlayerAttackAt\)\.toBe\(confirmedPlayerAttackAt\);[\s\S]*expect\(observedCritical\.at\)\.toBeGreaterThan\(observedCritical\.criticalPlayerAttackAt\);/,
  'critical capture must replenish and re-stabilize the durable room, close its final browser boundary, then require one exact runtime-confirmed source and prove the browser-locked source matches it');
assert.match(journey, /criticalPlayerAttackAt !== expectedPlayerAttackAt[\s\S]*playerLastAttackTime !== expectedPlayerAttackAt[\s\S]*observedPlayerAttackAt !== expectedPlayerAttackAt[\s\S]*critical-player-attack-source-mismatch/,
  'critical capture must fail closed unless runtime critical source, authoritative snapshot and consumed-player marker match the exact browser-locked source');
...
assert.match(journey, /const replenishedCriticalRoom = await page\.evaluate\(\(\) => window\.__dungeonVeilRuntimeEvidence\?\.loadRoom\(1, 'solo'\) \?\? null\);[\s\S]*expect\(Number\(replenishedCriticalRoom\?\.livingEnemies \|\| 0\)\)\.toBeGreaterThan\(0\);[\s\S]*await waitForStableRoom\(page\);[\s\S]*await prepareLivePlayerAttackLine\(page\);[\s\S]*const captureBoundaryState = await captureBoundaryHandle\.jsonValue\(\);[\s\S]*expect\(captureBoundaryState\.playerLastAttackTime\)\.toBe\(captureBoundaryState\.observedPlayerAttackAt\);[\s\S]*const confirmedPlayerAttackAt = await triggerConfirmedPlayerAttack/,
  'critical capture must replenish and re-prove a durable authoritative target before the final strict capture boundary and confirmed input');
...
assert.ok(
  ...
  'target replenishment and re-stabilization must finish before the final strict pre-input boundary so reload lifecycle cannot inject stale source history',
);
...
assert.match(journey, /expect\(confirmedPlayerAttackAt\)\.toBeGreaterThan\(captureBoundary\);/,
  'the confirmed player attack must advance beyond the final pre-input capture boundary');
assert.match(journey, /expect\(observedCritical\.criticalPlayerAttackAt\)\.toBe\(confirmedPlayerAttackAt\);/,
  'the accepted critical feedback must identify the exact independently confirmed special-producing player attack');
assert.match(journey, /expect\(observedCritical\.playerLastAttackTime\)\.toBe\(confirmedPlayerAttackAt\);[\s\S]*expect\(observedCritical\.observedPlayerAttackAt\)\.toBe\(confirmedPlayerAttackAt\);[\s\S]*expect\(observedCritical\.expectedPlayerAttackAt\)\.toBe\(confirmedPlayerAttackAt\);/,
  'the accepted payload must preserve exact convergence of snapshot, consumed-player marker and browser-locked critical source');
assert.match(journey, /expect\(observedCritical\.diagnosticActionAgeMs\)\.toBeGreaterThanOrEqual\(0\);[\s\S]*expect\(observedCritical\.diagnosticActionAgeMs\)\.toBeLessThanOrEqual\(COMPANION_FEEDBACK_CAPTURE_MAX_AGE_MS\);[\s\S]*expect\(observedCritical\.maxActionAgeMs\)\.toBe\(COMPANION_FEEDBACK_CAPTURE_MAX_AGE_MS\);/,
  'successful critical evidence must reassert the unchanged strict timing bound before receipt emission');
assert.match(journey, /schema: 'dungeon-veil-critical-support-timing-v1',[\s\S]*diagnosticActionAgeMs: observedCritical\.diagnosticActionAgeMs,[\s\S]*maxAgeMs: observedCritical\.maxActionAgeMs,[\s\S]*criticalPlayerAttackAt: observedCritical\.criticalPlayerAttackAt,[\s\S]*playerLastAttackTime: observedCritical\.playerLastAttackTime,[\s\S]*observedPlayerAttackAt: observedCritical\.observedPlayerAttackAt,[\s\S]*expectedPlayerAttackAt: observedCritical\.expectedPlayerAttackAt,[\s\S]*actionRole: observedCritical\.role,[\s\S]*actionTargetId: observedCritical\.targetId,[\s\S]*feedbackId: observedCritical\.feedbackId,[\s\S]*feedbackRole: observedCritical\.feedbackRole,[\s\S]*feedbackTargetId: observedCritical\.feedbackTargetId/,
  'the durable timing receipt must preserve the concrete freshness, source equality and identity tuple');
assert.match(journey, /DUNGEON_VEIL_CRITICAL_SUPPORT_TIMING_RECEIPT \$\{JSON\.stringify\(timingReceipt\)\}/,
  'the successful exact timing tuple must be emitted into the durable Product job log');
assert.match(journey, /expect\(observedCritical\.at\)\.toBeGreaterThan\(observedCritical\.criticalPlayerAttackAt\);/,
...
assert.match(journey, /'identity-mismatch'|'disconnected'|'no-correlated-action'|'action-age'|'not-visible-geometry'|'opacity-below-threshold'|'critical-player-attack-source-mismatch'/,
  'the capture must distinguish the first causal rejection boundary including exact critical-source mismatch without changing freshness acceptance');
