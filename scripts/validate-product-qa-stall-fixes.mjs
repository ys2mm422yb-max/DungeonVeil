...
const atomicPlayerAttackReader = companionJourney.match(/async function readConfirmedPlayerAttack\(page, attackBoundary\) \{[\s\S]*?\n\}/)?.[0] ?? '';
assert.match(atomicPlayerAttackReader, /page\.evaluate\(boundary => \{[\s\S]*window\.__dungeonVeilRuntimeEvidence\?\.snapshot\(\) \?\? null[\s\S]*companion-runtime-bridge[\s\S]*playerLastAttackTime = Number\(snapshot\?\.playerLastAttackTime \|\| 0\)[\s\S]*observedPlayerAttackAt[\s\S]*criticalPlayerAttackAt[\s\S]*playerLastAttackTime > boundary[\s\S]*observedPlayerAttackAt === playerLastAttackTime[\s\S]*criticalPlayerAttackAt === playerLastAttackTime[\s\S]*return \{ snapshot, confirmedAt, playerLastAttackTime, observedPlayerAttackAt, criticalPlayerAttackAt \};/,
  'the postcondition reader must observe one exact converged snapshot/consumed/critical timestamp without mutating browser observer state');
...
assert.match(companionJourney, /const exactPlayerAttackAt = \([\s\S]*playerLastAttackTime === criticalPlayerAttackAt[\s\S]*observedPlayerAttackAt === criticalPlayerAttackAt[\s\S]*\) \? criticalPlayerAttackAt : 0;[\s\S]*if \(!Number\.isFinite\(expectedPlayerAttackAt\)\) \{[\s\S]*scope\[expectedPlayerAttackStateKey\] = exactPlayerAttackAt;[\s\S]*expectedPlayerAttackAt = exactPlayerAttackAt;[\s\S]*\}[\s\S]*criticalPlayerAttackAt !== expectedPlayerAttackAt[\s\S]*playerLastAttackTime !== expectedPlayerAttackAt[\s\S]*observedPlayerAttackAt !== expectedPlayerAttackAt/,
  'the browser-local observer must atomically derive, lock and fail closed on one exact Critical Support source before freshness acceptance');

const playerAttackTrigger = companionJourney.match(/async function triggerConfirmedPlayerAttack\(page, attackBoundary\) \{[\s\S]*?\n\}/)?.[0] ?? '';
assert.match(playerAttackTrigger, /const inputBurst = 6;[\s\S]*readConfirmedPlayerAttack\(page, attackBoundary\)[\s\S]*livingEnemyPositions[\s\S]*moveWithKeyboard\(page, keys, durationMs\)[\s\S]*page\.keyboard\.press\('Space'\)[\s\S]*confirmedAt > attackBoundary/,
...
assert.match(playerAttackTrigger, /await moveWithKeyboard\(page, keys, durationMs\);[\s\S]*const movementState = await readConfirmedPlayerAttack\(page, attackBoundary\);[\s\S]*const movementAttackAt = Number\(movementState\.confirmedAt \|\| 0\);[\s\S]*if \(movementAttackAt > attackBoundary\) return movementAttackAt;[\s\S]*await page\.keyboard\.press\('Space'\);/,
  'the bounded search must return the first exact source observed during target approach before issuing another attack input');
assert.match(playerAttackTrigger, /const phase = attempt % 3;[\s\S]*\{ x: dx, y: dy \}[\s\S]*\{ x: -dy, y: dx \}[\s\S]*\{ x: dy, y: -dx \}/,
...
assert.match(companionJourney, /expect\(observedCritical\.playerLastAttackTime\)\.toBe\(confirmedPlayerAttackAt\);[\s\S]*expect\(observedCritical\.observedPlayerAttackAt\)\.toBe\(confirmedPlayerAttackAt\);[\s\S]*expect\(observedCritical\.expectedPlayerAttackAt\)\.toBe\(confirmedPlayerAttackAt\);/,
  'the accepted critical payload must preserve exact source convergence including the browser-locked source');
assert.match(companionJourney, /expect\(observedCritical\.diagnosticActionAgeMs\)\.toBeGreaterThanOrEqual\(0\);[\s\S]*expect\(observedCritical\.diagnosticActionAgeMs\)\.toBeLessThanOrEqual\(COMPANION_FEEDBACK_CAPTURE_MAX_AGE_MS\);[\s\S]*expect\(observedCritical\.maxActionAgeMs\)\.toBe\(COMPANION_FEEDBACK_CAPTURE_MAX_AGE_MS\);/,
  'successful Critical Support evidence must reassert the unchanged strict 250ms timing bound');
assert.match(companionJourney, /schema: 'dungeon-veil-critical-support-timing-v1',[\s\S]*diagnosticActionAgeMs: observedCritical\.diagnosticActionAgeMs,[\s\S]*maxAgeMs: observedCritical\.maxActionAgeMs,[\s\S]*criticalPlayerAttackAt: observedCritical\.criticalPlayerAttackAt,[\s\S]*playerLastAttackTime: observedCritical\.playerLastAttackTime,[\s\S]*observedPlayerAttackAt: observedCritical\.observedPlayerAttackAt,[\s\S]*expectedPlayerAttackAt: observedCritical\.expectedPlayerAttackAt,[\s\S]*actionRole: observedCritical\.role,[\s\S]*actionTargetId: observedCritical\.targetId,[\s\S]*feedbackId: observedCritical\.feedbackId,[\s\S]*feedbackRole: observedCritical\.feedbackRole,[\s\S]*feedbackTargetId: observedCritical\.feedbackTargetId/,
  'the durable timing receipt must preserve freshness, exact source equality and action/DOM identity');
assert.match(companionJourney, /DUNGEON_VEIL_CRITICAL_SUPPORT_TIMING_RECEIPT \$\{JSON\.stringify\(timingReceipt\)\}/,
  'successful exact timing evidence must be emitted into the durable Product job log');
