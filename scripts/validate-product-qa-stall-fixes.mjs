...
assert.match(companionJourney, /const attackBoundary = Number\(\(await readRuntimeCombatSnapshot\(page\)\)\?\.playerLastAttackTime \|\| 0\);[\s\S]*const capturePromise = captureLiveCompanionFeedbackEvidence\(page, \{[\s\S]*role: 'critical-support'[\s\S]*notBefore: attackBoundary[\s\S]*__dungeonVeilCriticalCompanionFeedbackObservationArmed[\s\S]*const atomicReadyBoundaryHandle = await page\.waitForFunction\(\(\{ setter \}\) => \{[\s\S]*data-critical-special-ready[\s\S]*const evidenceBoundary = performance\.now\(\);[\s\S]*window\[setter\]\?\.\(evidenceBoundary\) !== true[\s\S]*playerLastAttackTime[\s\S]*return \{ evidenceBoundary, playerLastAttackTime \};[\s\S]*__dungeonVeilCriticalCompanionFeedbackObservationSetMinimumAt[\s\S]*timeout: 20_000,[\s\S]*polling: 16[\s\S]*const atomicReadyBoundary = await atomicReadyBoundaryHandle\.jsonValue\(\);[\s\S]*const readyAttackBoundary = Number\(atomicReadyBoundary\.playerLastAttackTime \|\| 0\);[\s\S]*const confirmedPlayerAttackAt = await triggerConfirmedPlayerAttack\(page, Math\.max\(readyAttackBoundary, evidenceBoundary\)\);[\s\S]*window\[setter\]\?\.\(confirmedAt\) === true[\s\S]*const observedCritical = await capturePromise;[\s\S]*expect\(confirmedPlayerAttackAt\)\.toBeGreaterThan\(readyAttackBoundary\)[\s\S]*expect\(confirmedPlayerAttackAt\)\.toBeGreaterThan\(evidenceBoundary\)[\s\S]*expect\(observedCritical\.at\)\.toBeGreaterThan\(confirmedPlayerAttackAt\)/,
  'critical evidence must atomically close its browser acceptance boundary at real cooldown readiness and require the confirmed player attack to advance beyond both ready and evidence boundaries before any accepted critical feedback');
