...
assert.match(companionJourney, /async function readConfirmedPlayerAttackAndArm\(page, attackBoundary, expectedPlayerAttackSetterKey\) \{[\s\S]*window\.__dungeonVeilRuntimeEvidence\?\.snapshot\(\) \?\? null[\s\S]*const confirmedAt = Number\(snapshot\?\.playerLastAttackTime \|\| 0\);[\s\S]*data-last-critical-special-player-attack-at[\s\S]*data-last-observed-player-attack-at[\s\S]*criticalSpecialAt > boundary[\s\S]*criticalSpecialAt === confirmedAt[\s\S]*observedPlayerAttackAt === criticalSpecialAt[\s\S]*window\[setterKey\]\?\.\(confirmedAt\) === true[\s\S]*return \{ snapshot, confirmedAt, armed \};/,
  'the critical path must observe the authoritative player attack and arm that exact timestamp only after runtime independently confirms the same special-producing and consumed source');
