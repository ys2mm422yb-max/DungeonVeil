...
- `player_mailbox` – private per-player guild/friend requests, rewards, notices and important system information
- `friend_requests` – authenticated incoming and outgoing friendship requests
- `friendships` – symmetric accepted friend relationships, stored once per viewing direction
...
All public tables have RLS enabled. Anonymous table access is revoked. Players can only read their own mailbox, their own friendship data and permitted guild records. Guild invitation tokens are stored only as SHA-256 hashes and can be created or claimed through authenticated security-definer RPCs. Friend writes are also restricted to authenticated RPCs. World-boss health, hits, contributions and rewards cannot be written directly by the client.
...
Friendship requests also create actionable mailbox items. The receiver can accept or decline from either the mailbox or the friends panel. Accepted requests create the relationship in both viewing directions, while outgoing requests can be cancelled and existing friends can be removed. A unique unordered-pair index prevents duplicate cross-requests.
...
The frontend only contains the publishable project key. Never add a service-role or secret key to this repository.

## Applied migrations

The live project contains named migrations for profiles/cloud saves, guilds, secure guild invitation links, the private player mailbox, friendships, world-boss tables, RLS, private guild helpers, invitation acceptance, hit rate limiting and RLS/index performance optimization. Supabase security advisors are checked after DDL changes.
